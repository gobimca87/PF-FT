...
| SLM (hosted-first → target) | **Azure AI Foundry** (in-tenancy Azure) → internal self-hosted SLM (vLLM on AKS GPU). Hugging Face Inference API is retained as an **eval/experimentation provider only** (`ADR-D3-29`, supersedes `ADR-D3-13`; behind the `ADR-D3-14` abstraction) |
