...
- The AI platform **never**: authenticates or authorizes a request itself (APIM/enterprise auth does that — AI only consumes validated claims), re-implements business/compliance rules, writes directly to the enterprise database, invents portal URLs, silently guesses at failed/ambiguous transaction outcomes, lets a model output become an authorization decision, or exposes raw enterprise/personal data to an **external** SLM — external-SLM payloads are masked/tokenised by default and fail-closed (`ADR-D6-19`, refining `ADR-D6-07`); a **self-hosted** SLM may use raw or masked data since it stays in-tenancy.
...
| SLM (hosted-first → target) | **Azure AI Foundry** (in-tenancy Azure) → internal self-hosted SLM (vLLM on AKS GPU). Evaluation/experimentation also runs in-tenancy on Foundry, so there is **no active external SLM path**; the Hugging Face Inference API is kept only as a **dormant abstraction adapter** (`ADR-D3-29`, supersedes `ADR-D3-13`; behind the `ADR-D3-14` abstraction) |
| Embedding model | General-purpose 768-dim (`bge-base-en-v1.5` class or Azure OpenAI `text-embedding-3`), **hosted on Azure AI Foundry** (`ADR-D3-23` model choice; `ADR-D3-29` hosting) |
...
Memory/session/cache store is **resolved: Azure Managed Redis** (`ADR-D4-10`; supersedes `docs/adr/0004`). Deployment & delivery are **resolved: conform to the Enterprise Application delivery model** — Azure DevOps `build.yaml`/`release.yaml` CI/CD on the shared enterprise **AKS** platform, same platform team and resources, with the enterprise **SonarQube** quality gate; PFF AI stands up **no** separate infra/CI/CD/deployment stack, and any future tooling proposal defers to the enterprise standard (`ADR-D5-20`, Accepted; this ratifies the IaC decision `ADR-D5-12` and the Kubernetes decision `ADR-D5-13`, both now Accepted, and is the realizing toolchain for the CI/CD ADRs `ADR-D7-09`/`D7-10`/`D7-11`). Three further decisions carry a complete ADR evaluation and a **stated recommendation that is the working default for implementation — build against it, don't silently pick a different option**: embedding model → general-purpose 768-dim `bge-base-en-v1.5` class (`ADR-D3-23`), vector store → Azure AI Search (`ADR-D3-24`), self-hosted SLM serving stack → vLLM on AKS GPU (`ADR-D5-10`). Each of these three is still `status: Proposed`, not `Accepted` — formal ratification (an evaluation, a benchmark, or a named sign-off, per `ADR-D0-04`) is pending; deviating from the recommendation requires a superseding ADR, never a silent choice. Three newer decisions are also `Proposed` and awaiting first ARB sign-off — the runtime **quality-gated refinement loop** with a model-escalation ladder and strict mode (`ADR-D3-28`), the **SLM input masking regime** (`ADR-D6-19`), and the **model serving plane → Azure AI Foundry** hosted-first (in-tenancy Azure) with self-hosted vLLM as target (`ADR-D3-29`, which **supersedes `ADR-D3-13`** and also sets the hosting plane for the `ADR-D3-23` embedding model; evaluation also runs in-tenancy on Foundry so there is **no active external SLM path** — the HF Inference API is only a dormant abstraction adapter) — build to their stated direction meanwhile. Full evaluations and the sign-off each is awaiting are in `docs/architecture/adr/_register/open-decisions.md`. See `DEVELOPMENT-GUIDE.md` §2 for the reconciliation notes (5-stage environment model, `AffiliationAgent`-only first).
