...
from pf_ft_ai.configuration.models import RedisConnectionSettings
from pf_ft_ai.infrastructure.redis_client import build_redis_client


def test_should_build_a_redis_client_from_connection_settings() -> None:
    settings = RedisConnectionSettings(
        host="pf-ft-redis.example", port=10000, ssl=True, db=0, password="super-secret"
    )
    ...
    assert connection_kwargs["host"] == "pf-ft-redis.example"
