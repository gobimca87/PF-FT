...
source_docs:
  - "MD files/1 Foundation/3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.1, §5.2, §65"
  - "MD files/0 Workflow/pff_affiliation_e2e_flow.md — Phases 0, 1, 6; Notification Summary"
  - "MD files/2 Agent Runtime/6 PFF-FA-AI-CONVERSATION-SESSION.md §1"
  - "MD files/Examples/SampleWorkflowchat.md"
build_phases: [3, 5, 10, 23]
impacted_paths:
  - src/pff_fa_ai/application/session/
  - prompts/persona/
...
are decided by APIM claims before the request reaches the platform (3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.2).
...
context. 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.2 is explicit that the AI platform does not override an APIM authorization
decision, and 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 lists SLM-controlled authorization as a never. But that prohibition is
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Communication style must adapt to who is being spoken to | `SampleWorkflowchat.md`; 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §65 |
| DR-F-02 | What a user may see and do must derive from validated enterprise claims only | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.2 |
| DR-F-03 | The platform must never derive access from conversational content | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48; ADR-D1-02 I-2 |
...
| DR-N-02 | Persona selection must be auditable | Every turn's persona recorded in the trace | 20.PFF-FA-AI-GOVERNANCE.md §29 |
| DR-N-03 | The model must scale to further workflows without new archetypes per workflow | Archetypes are role-based, not workflow-based | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §49 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | APIM validates identity and authorization; the platform consumes claims and never derives them | Platform | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.1, §5.2 |
| DR-C-02 | The platform must not independently authenticate a user | Platform | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.1 |
| DR-C-03 | The AI platform does not override an APIM authorization decision | Platform | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.2 |
...
|---|---|---|---|---|
| EC-01 | Impossibility of persona influencing access | 35 | The failure this decision exists to prevent; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 makes it categorical | Can any conversational signal change what data is assembled? |
...
- Creates exactly the coupling this decision exists to prevent. Once one field drives both, a
  change to it for stylistic reasons changes access, and the boundary in 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.2 depends on
...
- Makes the prohibition in 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 unenforceable by design rather than by policy.
...
rather than 1 because it drives only style, but the mechanism it creates is the one 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48
...
```mermaid
flowchart TD
    A[Request arrives at FastAPI<br/>with APIM-validated claims] --> B[Extract identity context<br/>3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.1]
...
| Repository paths | `src/pff_fa_ai/application/session/`, `src/pff_fa_ai/orchestration/harness/`, `prompts/persona/` |
...
| Specification sections | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §5.1 (Authentication), §5.2 (Authorization), §65 (Responsibility for User Communication); 6 PFF-FA-AI-CONVERSATION-SESSION.md §1 (Purpose); affiliation flow Phases 0, 1, 6, Notification Summary; `Examples/SampleWorkflowchat.md` |
| Requirement IDs | Per ADR-D1-12 |
| Build phases | 3, 5, 10, 23 |
| Code paths | `src/pff_fa_ai/application/session/`, `src/pff_fa_ai/orchestration/harness/`, `prompts/persona/` |
