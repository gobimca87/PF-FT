...
Exactly one agent is built: `AffiliationAgent`. The wider catalogue stays unfinalised, per 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md
...
7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §6 names seven agents and then states that the actual catalogue will be finalised
...
1 PF-FT-AI-ARCHITECTURE.md §39's twentieth success criterion is explicit that this must not happen: *"New
workflow-level agents can be added without redesigning the platform core."* 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §49's
...
admit multiple agents* — cheap, and required by 1 PF-FT-AI-ARCHITECTURE.md §39 — from *machinery for hypothetical
...
| DR-F-02 | New agents must be addable without redesigning the platform core | 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20; 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §49 |
| DR-F-03 | Agents are logical capabilities in one runtime, not microservices | 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48; `CLAUDE.md` |
| DR-F-04 | An agent owns one business interaction, not one API | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7 |
| DR-F-05 | The supervisor must route to a candidate agent, even when there is one candidate | 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 2; 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §9 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Adding agent two must not change orchestration code | 0 core changes | 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20 |
...
|---|---|---|---|
| DR-C-01 | The agent catalogue is unfinalised and must not be invented | Organisational | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §6; `DEVELOPMENT-GUIDE.md` §2 |
| DR-C-02 | One microservice per agent is an anti-pattern unless independently justified | Platform | 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48 |
| DR-C-03 | Agents may not execute business rules, access databases, bypass tools, override authorization or execute unrestricted APIs | Platform | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §5 |
...
|---|---|---|---|---|
| EC-01 | Extensibility without core redesign | 35 | 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20 is a stated success criterion, not a preference | Would adding agent two change orchestration code? |
...
| EC-05 | Fidelity to the specification's agent model | 8 | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7's granularity rule is binding | One agent per business interaction? |
| | **Total** | **100** | | |
...
**Weaknesses.**
- Fails 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20 outright. Adding agent two would require rebuilding the
...
- Extensibility is testable with a synthetic test-only second agent, without shipping one
  (EC-04).
- Matches 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7's granularity: one agent, one business interaction (EC-05).
...
**Weaknesses.**
- Directly violates 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7: "one agent = one API" is the anti-pattern it names, against "one
...
- 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48 lists one-microservice-per-agent as an anti-pattern; this is its logical
...
a second agent. C is in any case excluded by DR-C-01. D is excluded by 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7 regardless of
score.
...
|---|---|---|
| `AffiliationAgent` | One agent, one business interaction, per 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §7 | The committed workflow |
...
| Supervisor routing | Intent → candidate agents → selection | 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 2 requires routing; also handles out-of-scope intents, which exist from day one |
...
- No agent-to-agent handoff protocol. 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md's model is supervisor-routed, not agent-chained;
...
- No separate deployable per agent — DR-C-02 and 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48 forbid it absent a specific
  operational justification, and none exists.

### 7.3 Testing the extension claim without shipping a second agent

1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20 needs verification, and one agent cannot verify it. A **test-only
...
Not by this decision. The catalogue remains open per 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §6 and DR-C-01. It is finalised when:
...
Per DR-C-02 and 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48, agents are capabilities inside one runtime. `AffiliationAgent` is a
...
2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48 demands. It is not a default and not a growth path.

## 9. Consequences

### 9.1 Positive

- Adding agent two touches no orchestration code, satisfying 1 PF-FT-AI-ARCHITECTURE.md §39 criterion 20.
...
|---|---|
| Enterprise decides; AI orchestrates | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §5's prohibitions are the agent contract's boundary: an agent may not execute business rules, access databases, bypass tools, override authorization or execute unrestricted APIs. The contract encodes them. |
...
| RSK-06 | An agent is later given its own deployable without the 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48 justification | Low | Medium | Low | §8.3; a separate ADR required | AI Platform Owner | Low |
...
| RT-06 | An agent is proposed for independent deployment | Architecture review | Requires its own ADR with 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §48's operational justification |
...
| Specification sections | 7 PF-FT-AI-AGENTIC-ORCHESTRATION.md §5 (agent prohibitions), §6 (Workflow-Level Agent Responsibility — the seven examples and "finalized separately"), §7 (Why Workflow-Level Agents); 1 PF-FT-AI-ARCHITECTURE.md §10 (Workflow-level Agents), §39 criterion 20; 2. PF-FT-AI-ARCHITECTURE-DETAILED.md §9 (Supervisor Layer), §48 (Anti-Patterns), §49 (Extension Model); `DEVELOPMENT-GUIDE.md` §2 |
