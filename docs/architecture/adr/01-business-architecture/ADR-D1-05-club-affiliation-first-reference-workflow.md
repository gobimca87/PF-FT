...
small number of capabilities — long-running execution across HTTP requests (doc 2 §28), HIL
suspension and resume (doc 2 §29), event-driven continuation (doc 2 §26), ERC batching (doc 2
§16), transaction uncertainty (doc 8 §66). A read-only workflow exercises none of them. The
platform would appear to work and would collapse on contact with the first workflow that
waits for a human.

Doc 1 §37 and doc 2 §47 both already designate Club Affiliation as the reference flow, and
doc 3 §48 works through the responsibility split for it. The 654-line flow document exists.
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | The first workflow must exercise long-running execution across multiple requests | doc 1 §39 criterion 13; doc 2 §28 |
| DR-F-02 | It must exercise human-in-the-loop suspension and resume | doc 2 §29; affiliation Phase 6 PENDING CFA |
| DR-F-03 | It must exercise event-driven ERC refresh and workflow resume | doc 1 §39 criterion 12; affiliation Phase 6-7 |
| DR-F-04 | It must exercise ERC aggregation across several enterprise services with batching | doc 1 §39 criteria 5-6; affiliation Phase 1 |
| DR-F-05 | It must include cases where a transaction outcome is genuinely uncertain | doc 8 §66; affiliation Scenarios 21-27 |
...
| DR-N-03 | Capabilities built must be reusable, not affiliation-specific | ≥80% of components reused by workflow two | doc 2 §49 |
...
- Already designated as the reference flow by doc 1 §37 and doc 2 §47.
...
already made in doc 1 §37 and doc 2 §47, with the rationale those documents did not record.
...
it, which is why doc 2 §48 lists in-memory-only long-running workflows as an anti-pattern.
...
- Confirms a designation already made in doc 1 §37 and doc 2 §47; the decision here is the
...
| RT-03 | QM-06 records a safeguarding fact without provenance | Weekly audit | Immediate governance incident per doc 20 §105 |
...
| Specification sections | `MD files/0 Workflow/pff_affiliation_e2e_flow.md` Phases 0–11, Complete Scenario Summary Table, Application Status Reference, Key Decision Flags; doc 1 §37 (Club Affiliation Reference Architecture), §39 (Success Criteria); doc 2 §47 (Club Affiliation Reference Flow), §49 (Extension Model); doc 3 §48 (Responsibility During Affiliation) |
