...
Doc 3 §63 states the ordering. Doc 3 §47 gives five conflict-resolution rules covering the
specific cases. Doc 8 §19 defines ERC authority levels. What none of them records is why this
...
There is also a scoping question the specifications address but easily gets lost. Doc 2 §3.6
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Conflicts between sources must resolve deterministically and identically on every turn | doc 3 §47 |
| DR-F-02 | Enterprise operational state must win over any AI-held copy | doc 3 §47 Rule 1, Rule 4 |
| DR-F-03 | Model output must never be treated as an authoritative data source | doc 3 §63 |
| DR-F-04 | RAG content must never be presented as operational truth | doc 2 §3.6; doc 8 §7 |
| DR-F-05 | Every fact in context must carry its source, so precedence is computable | doc 8 §15, §16 (Provenance) |
| DR-F-06 | A human enterprise decision (HIL) outranks any AI suggestion | doc 3 §47 Rule 5 |
...
| DR-N-02 | Conflicts must be observable, not silently resolved | 100% of resolved conflicts logged | doc 20 §29 |
| DR-N-03 | Freshness of authoritative state must be adequate for the workflow | Per-section freshness policy honoured | doc 8 §17, §18 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | The ordering is fixed by specification and is not open for local variation | Organisational | doc 3 §63; `CLAUDE.md` |
| DR-C-02 | ERC is not the enterprise database, not memory, not cache and not RAG | Platform | doc 8 §4, §5, §6, §7 |
| DR-C-03 | SLM output is never authoritative under any circumstance | Platform | doc 3 §63 |
| DR-C-04 | Transaction outcomes may be genuinely uncertain and must not be resolved by inference | Platform | doc 8 §66 |
...
- Directly implements doc 3 §63 and doc 8 §15–§19.
...
**Cost / effort.** Moderate; provenance is already required by doc 8 §15–§16.
...
- Permits SLM output to win a conflict if its confidence is high enough, violating DR-C-03
  and doc 3 §63 categorically. Model confidence is in any case a poor predictor of
...
§2 and the five rules in doc 3 §47.
...
competes with it. Doc 2 §3.6 requires the two categories to remain distinguishable, and the
...
An ERC section past its freshness policy (doc 8 §17–§18) is not demoted to cache authority —
...
current. Doc 3 §50 governs API-failure responsibility; ADR-D3-08 governs the wording.
...
### 7.5 Doc 3 §47's five rules as instances

The five conflict rules in doc 3 §47 are consequences of §7.1, not separate rules:

| Doc 3 §47 rule | Instance of |
|---|---|
...
Doc 8 §66 identifies a distinct case: a transaction whose outcome is genuinely unknown —
...
Every fact entering context carries provenance, per doc 8 §15–§16:
...
ContextFact
  ...
  freshness_policy the section's policy, per doc 8 §18
...
- RAG cannot leak into operational answers, satisfying doc 2 §3.6.
- Doc 3 §47's five rules become derivable rather than enumerated, so unforeseen conflict
...
- The ordering itself was fixed by doc 3 §63; this decision concerns its implementation and
...
| Audit and evidential requirements | Provenance on every fact gives a complete lineage from enterprise source to displayed statement, satisfying doc 20 §60 (Data Lineage) and §61 (Data Authority). |
...
| Effort estimate | Moderate — provenance is already mandated by doc 8 §15–§16, so the incremental work is the resolution and enforcement logic |
...
| AC-06 | Doc 3 §47's five rules each produce the specified outcome | Scenario tests, one per rule |
...
| Rollback | The ordering is fixed by doc 3 §63 and cannot be changed by configuration. `source-precedence.yaml` maps source types to levels; it cannot reorder them. |
...
|---|---|---|---|
| Provenance plumbing | Part of Phase 5 | — | Largely required by doc 8 §15–§16 regardless |
...
| RT-05 | Doc 3 §63 or doc 8 §19 amended | Change notice | Re-derive §7.1 and the authority mapping |
...
| Specification sections | doc 3 §47 (Conflict Resolution Rules), §54 (Responsibility During RAG Conflict), §55 (Responsibility During Cache Conflict), §63 (Ownership of Authoritative Truth); doc 2 §3.5 (Context Before Reasoning), §3.6 (Knowledge vs Operational Truth); doc 8 §7 (ERC Is Not RAG), §15–§16 (Source Provenance), §17–§18 (Freshness), §19 (ERC Authority Levels), §65 (ERC and Transaction State), §66 (Transaction Uncertainty) |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Precedence implemented as fact-level authority; scoped to operational truth only; freshness handled by invalidation rather than demotion; doc 3 §47's rules derived rather than enumerated. Tier 1 — ratified by the external ADF/ADR forum. |
