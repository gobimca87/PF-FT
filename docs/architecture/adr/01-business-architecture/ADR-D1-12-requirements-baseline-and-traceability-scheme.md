...
source_docs:
  - "MD files/1 Foundation/1 PFF-FA-AI-ARCHITECTURE.md §38, §39"
  - "MD files/1 Foundation/2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50"
  - "MD files/5 QualityGovernance/20.PFF-FA-AI-GOVERNANCE.md §115, §116"
  - "MD files/6 Production/26.PFF-FA-AI-PERFORMANCE-COST.md"
...
re-elicited. 1 PFF-FA-AI-ARCHITECTURE.md §39's twenty architecture success criteria become the functional baseline;
1 PFF-FA-AI-ARCHITECTURE.md §38's eleven quality attributes become the non-functional baseline once each is given a
target, since 1 PFF-FA-AI-ARCHITECTURE.md §38 lists the attributes and defers the numbers.

## 2. Context and Problem Statement

20.PFF-FA-AI-GOVERNANCE.md §115 mandates a traceability chain: requirement → architecture → implementation → test →
evaluation → release → evidence. 20.PFF-FA-AI-GOVERNANCE.md §116 makes it part of the governance definition of
...
- **1 PFF-FA-AI-ARCHITECTURE.md §39** gives twenty architecture success criteria, each a testable statement ("ERC
...
- **1 PFF-FA-AI-ARCHITECTURE.md §38** lists eleven quality attributes — availability, reliability, scalability,
...
- **2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50** defines "architecture complete" in similar terms.

So the gap is narrower than "no requirements exist". It is: the functional statements need
identifiers, the quality attributes need targets, and both need a scheme that makes 20.PFF-FA-AI-GOVERNANCE.md
...
statement of what the platform must do, alongside 1 PFF-FA-AI-ARCHITECTURE.md §39, 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50 and this library, and
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Every requirement must have a stable unique identifier | 20.PFF-FA-AI-GOVERNANCE.md §115 |
| DR-F-02 | The chain requirement → architecture → implementation → test → evaluation → release → evidence must be navigable | 20.PFF-FA-AI-GOVERNANCE.md §115 |
| DR-F-03 | Non-functional attributes must have measurable targets, not just names | 1 PFF-FA-AI-ARCHITECTURE.md §38 |
| DR-F-04 | A specification amendment must identify the requirements it affects | Programme practice |
| DR-F-05 | Requirements must not be re-elicited, creating a competing statement | 1 PFF-FA-AI-ARCHITECTURE.md §39; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50 |
...
| DR-N-03 | Traceability must be verifiable mechanically | Broken links detectable by a script | 20.PFF-FA-AI-GOVERNANCE.md §116 |
...
| DR-C-02 | 1 PFF-FA-AI-ARCHITECTURE.md §38 defers NFR targets to the NFR and performance documents | Organisational | 1 PFF-FA-AI-ARCHITECTURE.md §38 |
| DR-C-03 | 1 PFF-FA-AI-ARCHITECTURE.md §39's twenty criteria are the stated definition of architectural success | Organisational | 1 PFF-FA-AI-ARCHITECTURE.md §39 |
...
|---|---|---|---|
| DR-A-01 | 1 PFF-FA-AI-ARCHITECTURE.md §39's criteria are complete enough to serve as the functional baseline | Gaps are added as programme-derived requirements under §7.2's `FR-P-` prefix | Coverage review at Phase 17 |
| DR-A-02 | NFR targets can be set from 26.PFF-FA-AI-PERFORMANCE-COST.md and programme judgement without a separate elicitation | Targets are provisional until 26.PFF-FA-AI-PERFORMANCE-COST.md is fully mined | Reviewed at Phase 20 |
...
|---|---|---|---|---|
| EC-01 | Traceability chain navigability | 30 | 20.PFF-FA-AI-GOVERNANCE.md §115 and §116 make this a governance obligation | Can the chain be walked in both directions? |
...
- Creates a fourth statement of platform requirements alongside 1 PFF-FA-AI-ARCHITECTURE.md §39, 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50 and this
...
**Description.** Identifiers encode their origin: `FR-A39-01` for 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 1,
`NFR-A38-PERF` for the performance attribute in 1 PFF-FA-AI-ARCHITECTURE.md §38, `FR-P-nn` for requirements the
...
- 1 PFF-FA-AI-ARCHITECTURE.md §39's criteria predate this library and would need reassigning to ADRs artificially.
...
**Method.** Weighted scoring against §4. EC-02 tested against a concrete case: 8 PFF-FA-AI-ERC-CONTEXT.md §36 fixes
...
link of 20.PFF-FA-AI-GOVERNANCE.md §115's chain circular, and no reweighting repairs that.
...
| Source code | Origin |
|---|---|
| `A38` | 1 PFF-FA-AI-ARCHITECTURE.md §38 — quality attributes |
| `A39` | 1 PFF-FA-AI-ARCHITECTURE.md §39 — architecture success criteria |
| `AFF` | `MD files/0 Workflow/pff_affiliation_e2e_flow.md` |
| `GR` | The Golden Rule and its constraints (`CLAUDE.md`, 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §63) |
...
1 PFF-FA-AI-ARCHITECTURE.md §39's twenty architecture success criteria become `FR-A39-01` … `FR-A39-20`, cited not
restated. A representative extract:

| ID | Requirement (1 PFF-FA-AI-ARCHITECTURE.md §39) | Primary ADRs |
|---|---|---|
...
1 PFF-FA-AI-ARCHITECTURE.md §38 names eleven attributes and defers targets. This decision assigns each attribute an
...
"Target pending" is honest rather than deficient: 1 PFF-FA-AI-ARCHITECTURE.md §38 defers these to the performance and
...
response is that **an identifier is never reassigned**. If 1 PFF-FA-AI-ARCHITECTURE.md §39's criterion 5 becomes
...
identified, not any 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §52 architecture category — ratified by the AI Solution Architect.

## 8. Architecture Detail

### 8.1 The 20.PFF-FA-AI-GOVERNANCE.md §115 chain, instantiated

```mermaid
flowchart LR
    R["FR-A39-06<br/>ERC supports<br/>20-record batching"] --> A["ADR-D4-04<br/>batching decision"]
    A --> I["src/pff_fa_ai/context/<br/>collection/batching.py"]
...
8 PFF-FA-AI-ERC-CONTEXT.md §36 fixes the agreed ERC batch size at 20. Suppose it is amended to 50.
...
- 20.PFF-FA-AI-GOVERNANCE.md §115's chain becomes navigable and mechanically checkable.
...
| Immediate NFR targets | Not inventing numbers 1 PFF-FA-AI-ARCHITECTURE.md §38 explicitly defers | AI Product Owner |
...
| Audit and evidential requirements | This decision is what makes 20.PFF-FA-AI-GOVERNANCE.md §115 and §116 satisfiable. Without identifiers there is no chain to evidence. |
...
| AC-04 | 1 PFF-FA-AI-ARCHITECTURE.md §39's twenty criteria all appear as `FR-A39-01` … `FR-A39-20` | Baseline audit |
| AC-05 | All eleven 1 PFF-FA-AI-ARCHITECTURE.md §38 attributes appear with a named target-setting ADR | Baseline audit |
...
| Specification sections | 1 PFF-FA-AI-ARCHITECTURE.md §38 (Non-Functional Requirements — attributes named, targets deferred), §39 (Architecture Success Criteria); 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §50 (Definition of Architecture Complete); 20.PFF-FA-AI-GOVERNANCE.md §115 (Governance Traceability Matrix), §116 (Governance Definition of Done); 26.PFF-FA-AI-PERFORMANCE-COST.md (Performance & Cost); affiliation flow scenario table |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Source-anchored identifier scheme; 1 PFF-FA-AI-ARCHITECTURE.md §39 adopted as the functional baseline by reference; 1 PFF-FA-AI-ARCHITECTURE.md §38's eleven attributes given identifiers with targets assigned to the ADRs that set them; enterprise business rules explicitly excluded from the baseline. |
