...
and security evals — with **regression thresholds** blocking promotion (doc 21 §6–§26,
...
Doc 21 §6–§8 evaluation levels/types/offline, §10–§13 golden dataset/versioning/categories,
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Versioned golden datasets | doc 21 §10, §12 |
| DR-F-02 | Deterministic + subjective (judge) eval | doc 21 §16–§17, §18–§24 |
| DR-F-03 | Regression thresholds block promotion | doc 21 §26; doc 20 §86 |
| DR-F-04 | Adversarial + security eval | doc 21 §56–§58 |
| DR-C-01 | Judge validated vs human ratings | doc 21 §59 |

### 3.4 Assumptions

| ID | Assumption | If false | Validation |
|---|---|---|---|
| DR-A-01 | LLM-judge correlates with human ratings | Fall back to human eval for that dimension | doc 21 §59 |
...
**Description.** Versioned golden datasets per category (doc 21 §13); deterministic checks
...
**Description.** Option A plus online eval (doc 21 §9) on production traffic feeding new
...
|---|---|
| No eval gate | doc 20 §86 |
| Unversioned datasets | doc 21 §12 |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 21 §6–§26/§56–§59.
...
E).** The judge is calibrated against human ratings (doc 21 §59). Deterministic-only (B),
judge-only (C) and human-only (D) are rejected as sole approaches.

## 8. Architecture Detail

- `tests/eval/`: golden datasets per category (doc 21 §13), versioned (§12); deterministic
...
- Regression thresholds (§26) gate promotion in CI (ADR-D7-09) and LLMOps (ADR-D7-12);
  online eval (§9) samples production (privacy-safe, doc 20 §84) to grow datasets + detect
  drift.
...
| Data classification touched | Eval datasets governed (doc 20 §84) |
...
| Specification sections | doc 21 §6–§26, §56–§59 |
