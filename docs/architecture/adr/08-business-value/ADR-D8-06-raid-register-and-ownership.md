...
owner and status per entry, integrated with the existing risk register (doc 20 §18–§20),
open-decisions register (ADR-D0-04), governance exceptions (doc 20 §101) and per-ADR risk
sections — so programme-level RAID is tracked in one place and reviewed on a cadence.

## 2. Context and Problem Statement

Doc 20 §18–§20 AI risk register/treatment/residual, §101 governance exceptions. ADRs each
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Track R/A/I/D with owner + status | doc 20 §18–§20 |
| DR-F-02 | Integrate with risk/open-decision/exception registers | doc 20 §18, §101; ADR-D0-04 |
...
**Description.** A RAID register in `_register/` linking to the risk register (doc 20 §18),
open decisions (ADR-D0-04) and exceptions (§101); each entry has an owner + status; reviewed
on a cadence (doc 20 §90).
...
|---|---|
| No RAID tracking | doc 20 §18 |
| Duplicate risk data across tools | Sync errors |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 20 §18–§20/§90/§101 and the ADR
registers.
...
- Integrates: risks from ADR §11 + doc 20 §18; assumptions from ADR §3.4; open decisions
  (ADR-D0-04); exceptions (doc 20 §101, e.g. GOV-EX-ADR-001 from ADR-D0-01); issues from
  incidents (ADR-D7-17). Reviewed at governance cadence (doc 20 §90).
...
| Specification sections | doc 20 §18–§20, §90, §101 |
