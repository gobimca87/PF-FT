...
(doc 9 §33–§47). A hard rule: **transaction/command responses are never cached** as
truth (doc 9 §35); the cache sits below ERC/enterprise in precedence and never serves
stale data as authoritative.

## 2. Context and Problem Statement

Doc 9 §33–§34 define cache categories and the enterprise-API response cache; §35 sets
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Cache-aside for enterprise reads | doc 9 §41 |
| DR-C-01 | Never cache transaction/command results as truth | doc 9 §35 |
| DR-F-02 | Volatility-aware TTL + event invalidation | doc 9 §38–§39, §44 |
| DR-F-03 | Stampede/penetration/poisoning protection | doc 9 §45–§47 |
| DR-C-02 | Namespaced, isolation-safe keys | doc 9 §36–§37 |
| DR-C-03 | Cache ranks below ERC/enterprise | ADR-D1-03; doc 9 §58 |
...
|---|---|
| Cache transaction/command results | doc 9 §35 |
| Global (non-namespaced) cache keys | doc 9 §36–§37 — isolation |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 9 §33–§47/§57–§58 and the
precedence chain.
...
**Status rationale.** `Accepted` — doc 9 §33–§47 govern this.
...
| Specification sections | doc 9 §33–§47, §57–§58 |
