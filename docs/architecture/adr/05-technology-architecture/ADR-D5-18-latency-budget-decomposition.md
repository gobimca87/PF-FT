...
so latency regressions are attributable and enforceable (doc 26 §5–§11, §16, §39,
§52). Time-to-first-token and time-to-complete are tracked separately for streamed
responses.

## 2. Context and Problem Statement

Doc 26 §5–§9 define performance objectives, end-to-end latency, percentiles and the
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | End-to-end budget in percentiles | doc 26 §6–§8 |
| DR-F-02 | Per-hop sub-budgets with owners | doc 26 §8–§9 |
| DR-F-03 | TTFT vs time-to-complete for streaming | doc 26 §10–§11; ADR-D3-19 |
| DR-N-01 | Attributable regressions | doc 26 §52 |
...
**Weaknesses.** Means hide tail latency users feel; doc 26 §7 wants percentiles.
...
|---|---|
| No latency targets | doc 26 §5–§6 |
| Max-latency-only (no percentiles) | Hides typical UX |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 26 §5–§11/§16/§39/§52.
...
**Status rationale.** `Accepted` — doc 26 §5–§11 govern this.

## 8. Architecture Detail

- Budget table maps each hop to a p95 sub-budget + owner (doc 26 §9); the sum + overhead
  ≤ the end-to-end p95 target.
- Tracing (ADR-D7-02) tags spans per hop so SLIs are computed per hop; TTFT/complete
  (ADR-D3-19) tracked for streams; RAG breakdown (doc 26 §52) sub-decomposed
  (embed/search/rerank).
...
| Specification sections | doc 26 §5–§11, §16, §39, §52 |
| Requirement IDs | LAT-* |
| Build phases | 2, 8 |
