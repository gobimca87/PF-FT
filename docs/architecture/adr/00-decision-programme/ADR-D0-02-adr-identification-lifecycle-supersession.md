...
a superseding record rather than rewritten — the same immutability rule doc 20 §73 applies
...
it is silently wrong. Doc 20 §73 states the general principle for platform artefacts:
versions are immutable and changes are released, not mutated. Doc 2 §43 applies it to
...
| DR-F-03 | Decision history must survive revision — what was decided, when, and what replaced it | doc 20 §73, §114; doc 3 §73 |
| DR-F-04 | A significance test must exist so the library's scope is decidable rather than argued | ADR-D0-01 DR-A-03 |
| DR-F-05 | Change classification must align with the platform's existing change-management model | doc 20 §76, §77 |
...
| DR-N-03 | Link stability — a cited ADR path must not move | 0 renames after Accepted | doc 20 §114 |
...
|---|---|---|---|---|
| EC-01 | History integrity — can a superseded decision still be read as it stood? | 30 | The single failure this policy exists to prevent; doc 20 §73 makes immutability a platform-wide rule | Is the prior text recoverable without reading Git diffs? |
...
requirement in doc 20 §73 and §114. EC-02 to EC-04 are assessed against the library's known
...
**An Accepted ADR is never edited in substance.** This mirrors doc 20 §73 and doc 2 §43,
which forbid mutating a released version of any platform artefact.

Changes are classified as follows, aligning with doc 20 §77:
...
1. It touches one of the doc 2 §52 categories — system boundaries, data ownership, agent
...
**Status rationale.** Accepted. Concerns how decisions are recorded, not any doc 2 §52
...
| Versioned artefacts, never mutated in place in production | This ADR *is* that rule applied to decision records. §7.3 mirrors doc 20 §73 and doc 2 §43 exactly: immutable versions, changes released as new versions, prior versions retained. |
...
| Audit and evidential requirements | Substantial and positive. Immutability is what makes the library admissible as evidence: an auditor can be shown what was decided on a date, not a reconstruction. Supports doc 20 §30 (Auditability) and §99 (Compliance Evidence). |
...
| Specification sections | doc 20 §73 (Version Governance), §74 (Version Compatibility), §76 (Change Management), §77 (Change Classification), §114 (Governance Documentation Version); doc 3 §73 (Change Control); doc 2 §43 (Versioning Architecture), §52 (Architecture Governance) |
