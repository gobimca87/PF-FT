...
problem is deferral without a closing condition — which doc 20 §103 identifies precisely,
...
| DR-F-03 | Every deferral must carry a closing condition | doc 20 §103 (Permanent Exceptions) |
| DR-F-04 | Reaching the closing condition unresolved must escalate | doc 20 §104 (Governance Escalation) |
| DR-F-05 | An open decision must carry enough analysis that closing it is a choice, not a research project | doc 20 §19 (Risk Treatment) |
...
| DR-C-04 | Open architecture choices are built behind interfaces, so the choice stays reversible while open | Platform | `docs/adr/0004` §Context; doc 9 §137–§138 |
...
| EC-02 | Forces closure at the right time | 25 | Deferral without a closing condition is doc 20 §103's indefinite exception | Is there a specific, detectable event that forces the question? |
...
**Description.** Enter each unmade decision in the doc 20 §18 AI risk register, with
...
build phase by which it must close. The phase is the closing condition doc 20 §103 requires.
...
1. **Build behind an interface.** The open choice sits behind an abstraction — `VectorStore`,
   ...
   store and doc 9 §137–§138 requires.
...
Reaching a decision's closing phase without ratification is an escalation under doc 20 §104:
...
flowchart TD
    ...
    G --> H[Recorded as an exception per<br/>doc 20 §101-§102 with an expiry date]
    H --> I[Phase proceeds only on the parts that<br/>do not depend on the decision]
```

A formal deferral under §7.4 is a governance exception in doc 20 §101's sense and takes
that shape — reason, risk, compensating controls, owner, approver, start date, expiry,
review date. It is not a quiet slip of the date. Per doc 20 §103, a decision deferred twice
...
**Status rationale.** Accepted. Concerns how open decisions are tracked, not any doc 2 §52
...
doc 2 §52 deployment-boundary decision — so each now has its own record.
...
- Deferral becomes a governed act with an expiry, matching doc 20 §101–§103.
...
| Freedom to defer indefinitely | Bounded deferral with an expiry, per doc 20 §103 | AI Platform Owner |
...
| RSK-04 | Decisions deferred repeatedly, becoming doc 20 §103's permanent exception | Medium | High | High | §7.4 escalates a twice-deferred decision to the AI Platform Owner; every deferral carries an expiry | AI Platform Owner | Low |
...
| Audit and evidential requirements | Supports doc 20 §102 by giving every deferral the required shape — reason, risk, compensating controls, owner, approver, expiry. |
...
| Interface abstraction | Absorbed | — | Already required by doc 9 §137–§138 and existing practice |
...
| Specification sections | doc 20 §18 (AI Risk Register), §19 (Risk Treatment), §20 (Residual Risk), §101 (Governance Exceptions), §102 (Exception Requirements), §103 (Permanent Exceptions), §104 (Governance Escalation); doc 3 §73 (Change Control); doc 9 §137–§138 (Provider Independence) |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Open decisions as `Proposed` ADRs, phase-gated closing conditions, escalation per doc 20 §104, interface rule preserving reversibility. |
