...
Doc 14 §31 defines the vector database boundary and §32 lists ten candidate
...
retrieval-time ACL path (doc 14 §49, ADR-D6-12) has no concrete target.
...
queries expected. Doc 14 §36 notes ANN (HNSW/IVF/PQ) is "normally used for large
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Vector similarity search over 768-dim embeddings | doc 14 §35, §52; ADR-D3-23 |
| DR-F-02 | Metadata filtering incl. ACL/security metadata at query time | doc 14 §27, §49, §50 |
| DR-F-03 | Hybrid (lexical + vector) search for exact identifiers | doc 14 §57, §58; ADR-D3-22 |
| DR-F-04 | Filter-injection-safe filter construction | doc 14 §51 |
| DR-F-05 | Blue/green index + alias cutover for re-embedding | doc 14 §77, §78, §84 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Retrieval latency | p95 vector search ≤ 120 ms | doc 14 §88 |
| DR-N-02 | Availability | ≥ 99.9% (managed SLA) | doc 14 §33 |
| DR-N-03 | Private networking / no public data-plane | Private endpoint only | doc 14 §34, §122 |
| DR-N-04 | Identity-based access, no shared keys | Entra ID RBAC | doc 14 §34, §124 |
| DR-N-05 | Backup / DR | Managed backup + rebuild path | doc 14 §33, §160; §86 |
...
| DR-C-02 | Index holds knowledge/FAQ only — never enterprise business truth | Regulatory/Arch | doc 13 §5; ADR-D3-20 |
| DR-C-03 | Vectors are sensitive (invertible to text) — must be encrypted & access-controlled | Security | doc 14 §121, §122, §124 |
| DR-C-04 | Selection is an ADR decision, ship as Proposed until signed off | Organisational | doc 14 §32; ADR-D0-04 |
| DR-C-05 | Cache/memory store is a *separate* concern from the vector store | Architecture | doc 9; ADR-D4-10, ADR-D4-12 |
...
| DR-A-03 | 768-dim is fixed by ADR-D3-23 | Index dimension changes ⇒ new index | Dimension guard, doc 14 §18 |

## 4. Evaluation Criteria and Weights

Criteria fixed before scoring (CMMI DAR SP 1.1), drawn from doc 14 §33 and §34 and
...
Five candidates scored from the doc 14 §32 list, spanning managed-Azure-native,
...
- Azure-native: private endpoint, Entra RBAC, Key Vault, Monitor/App Insights,
  data residency — satisfies every doc 14 §34 enterprise consideration out of the
  box.
...
- HNSW in pgvector is capable but less turnkey than a search service; index tuning
  is manual.
- Risks blurring the doc 9 separation if the same Postgres is reused for other state
...
**Weaknesses.**
- **Conflates two architectural concerns** doc 9 keeps separate — the ephemeral
...
**Method.** Weighted scoring against §4, informed by doc 14 §33–§34 criteria, the
...
a blue/green-aliased index (doc 14 §77–§78). Hybrid lexical+vector search serves
exact-identifier retrieval (ADR-D3-22); OData metadata filters enforce ACLs at
query time (ADR-D6-12), constructed parametrically to resist filter injection (doc
14 §51).
...
- **Index shape** (doc 14 §35): dimension 768, cosine metric (matched to embedding,
...
- **Hybrid search** (doc 14 §57–§58): vector query + BM25 over the chunk text, fused
  by the service's ranker; the vector layer contributes semantic recall, lexical
  contributes exact-ID precision.
- **ACL at query time** (doc 14 §49–§51; ADR-D6-12): the caller's authorisation
...
- **Re-embedding / model change**: build a new index, embed into it, evaluate, then
  atomically repoint the alias (doc 14 §78, §84) — no in-place mutation.
- **Networking/identity** (doc 14 §34, §122, §124): private endpoint only; Entra ID
...
- Clean separation from the cache/memory plane (doc 9) preserved.
...
|---|---|
| Enterprise decides & executes; AI interprets/orchestrates | Vector store serves *knowledge* retrieval only; it holds no business truth and makes no decision (doc 13 §5, ADR-D3-20). |
...
| Configuration | Vector store + index config (doc 14 §129, §130); private-endpoint & RBAC in IaC (ADR-D5-12) |
...
| Specification sections | doc 14 §31–§37, §47–§51, §57–§58, §84–§86, §88, §91, §95, §122, §124, §160; doc 13 §5 |
| Requirement IDs | RAG-VDB-* (per ADR-D1-12 scheme) |
| Build phases | 8 |
| Code paths | `src/pf_ft_ai/rag/vector_store/` |
| Configuration | Vector store & index config (doc 14 §129–§130) |
| Tests | vector store integration + security + eval suites (doc 14 §144–§153) |
