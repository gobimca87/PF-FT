...
source_docs:
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16, §17, §18, §19, §20, §21, §22, §23, §24"
  - "MD files/5 QualityGovernance/20.PFF-FA-AI-GOVERNANCE.md §45, §46, §93"
build_phases: [4]
impacted_paths:
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16 gives the agent lifecycle, §17 the execution contract, §18 the execution context, §19
the agent result, §20 agent configuration, §21 versioning, §22–§23 the capability registry and an
example, §24 agent selection. 20.PFF-FA-AI-GOVERNANCE.md §45–§46 cover agent governance and capability boundaries;
...
20.PFF-FA-AI-GOVERNANCE.md §46 requires agent capability boundaries, and 20.PFF-FA-AI-GOVERNANCE.md §93 requires agent review. Both are
...
There is a second question. 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16's lifecycle and §21's versioning imply agents have states
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | An agent must declare its capabilities for registry-based selection | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §22–§24 |
| DR-F-02 | Agent capability boundaries must be defined and reviewable | 20.PFF-FA-AI-GOVERNANCE.md §46, §93 |
| DR-F-03 | The execution contract must be uniform across agents | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §17 |
| DR-F-04 | Agents must be versioned | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §21 |
| DR-F-05 | The contract must generalise beyond affiliation | ADR-D1-11 §7.3, AC-04 |
| DR-F-06 | Agent result must be a defined shape | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §19 |
...
| DR-N-02 | An agent's permitted reach must be determinable without reading its code | Reviewable from configuration | 20.PFF-FA-AI-GOVERNANCE.md §93 |
...
| DR-C-02 | Agents may not execute business rules, bypass tools or override authorization | Platform | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §5 |
...
|---|---|---|---|---|
| EC-01 | Reviewability of capability boundary | 30 | 20.PFF-FA-AI-GOVERNANCE.md §46 and §93 require it; a boundary that needs code reading is not reviewable in practice | Can reach be determined from configuration alone? |
...
- Capability boundary is not reviewable without reading the implementation and reasoning about
  reachability, which 20.PFF-FA-AI-GOVERNANCE.md §93's review cannot do reliably (EC-01 fails).
...
by 20.PFF-FA-AI-GOVERNANCE.md §46 and §93 in substance.
...
agent:
  ...
  version: 1.0.0
  status: active                      # 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16 lifecycle
  capability:
...
This is the agent's **permitted reach**, and it is what governance reviews under 20.PFF-FA-AI-GOVERNANCE.md §93. A
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §17–§19's contract, kept deliberately narrow:

```python
class Agent(Protocol):
    async def handle(
        self,
        context: AgentExecutionContext,  # 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §18
        capabilities: AgentCapabilities,  # ADR-D2-09 §7.1
    ) -> AgentResult: ...  # 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §19
```
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16's lifecycle and §21's versioning, composed with the release model:
...
release, and 20.PFF-FA-AI-GOVERNANCE.md §49's workflow-change governance applies.
...
2. The declaration is amended, reviewed under 20.PFF-FA-AI-GOVERNANCE.md §93, and released.
...
declaration a floor rather than a boundary, and 20.PFF-FA-AI-GOVERNANCE.md §46's capability boundary would mean
...
Individual agent declarations are separately subject to 20.PFF-FA-AI-GOVERNANCE.md §93's agent review, which is where
...
flowchart TD
    ...
    D -.reviewed by.-> G[20.PFF-FA-AI-GOVERNANCE.md §93<br/>agent review]
...
20.PFF-FA-AI-GOVERNANCE.md §93's agent review, under this decision, reads one file and can state:
...
- An agent's permitted reach is one reviewable configuration file, satisfying 20.PFF-FA-AI-GOVERNANCE.md §46 and §93.
...
|---|---|
| Enterprise decides; AI orchestrates | The declaration bounds what an agent may reach; 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §5's prohibitions are enforced by the harness resolving only declared capabilities. |
...
| RSK-02 | Declarations become permissive to avoid friction | Medium | High | High | Governance review of the declaration (20.PFF-FA-AI-GOVERNANCE.md §93) is where over-permission is caught; QM-02 tracks allowlist size | Security Owner | Medium |
...
| Repository paths | `src/pff_fa_ai/orchestration/harness/`, `src/pff_fa_ai/agents/affiliation/` |
...
| RT-05 | 20.PFF-FA-AI-GOVERNANCE.md §46 or §93 amended | Change notice | Re-derive what the declaration must carry |
...
| Specification sections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §16 (Agent Lifecycle), §17 (Agent Execution Contract), §18 (Agent Execution Context), §19 (Agent Result), §20 (Agent Configuration), §21 (Agent Versioning), §22–§23 (Agent Capability Registry, Example), §24 (Agent Selection), §5 (agent prohibitions); 20.PFF-FA-AI-GOVERNANCE.md §45 (Agent Governance), §46 (Agent Capability Boundaries), §49 (Workflow Change), §93 (Agent Review) |
| Requirement IDs | `FR-A39-03`, `FR-A39-20`, `NFR-A38-SEC` |
| Build phases | 4 |
| Code paths | `src/pff_fa_ai/orchestration/harness/`, `src/pff_fa_ai/agents/` |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Declaration-first: capability, context requirements, tools, prompts and limits declared in versioned configuration and enforced by the harness, so 20.PFF-FA-AI-GOVERNANCE.md §93's agent review reads one file rather than an implementation. Runtime capability negotiation rejected because it would make the reviewed declaration a floor rather than a boundary. |
