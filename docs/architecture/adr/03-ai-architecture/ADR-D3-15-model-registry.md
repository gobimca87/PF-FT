...
DEPRECATED/RETIRED/BLOCKED) and compatibility metadata (doc 15 §8–§12, §154–§155;
doc 14 §15–§16). Model routing (ADR-D3-16), fallback (ADR-D3-18) and provider
selection (ADR-D3-14) all read from this single registry; no model is callable
unless it is registered and ACTIVE.

## 2. Context and Problem Statement

Doc 15 §8–§12 define a model registry with responsibilities, status, capability and
purpose; §151–§155 cover promotion, release artefact and immutable production
versions; doc 14 §15–§16 mirror this for embeddings. Without a formal registry,
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Resolve model by id@version with capability+purpose | doc 15 §8–§12 |
| DR-F-02 | Drive routing and fallback from declared capabilities | doc 15 §56, §58; ADR-D3-16/18 |
| DR-F-03 | Status lifecycle gates which models are usable | doc 15 §10 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Immutable production model versions | No in-place edit | doc 15 §155 |
| DR-N-02 | Auditable promotion | Versioned change history | doc 15 §151–§152 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | Registry is a versioned artefact | Organisational | CLAUDE.md; doc 15 §154 |
| DR-C-02 | Only registered+ACTIVE models callable (allowlist) | Security | doc 15 §121; ADR-D3-14 |
| DR-C-03 | Same model+version for doc & query embeddings | Platform | doc 14 §7; ADR-D3-23 |
...
|---|---|---|---|---|
| EC-01 | Immutability & version integrity | 24 | doc 15 §155 | Runtime mutation possible? |
...
**Description.** `config/models/*.yaml` (doc 15 §15 shape) versioned in Git, promoted
...
**Method.** Weighted scoring against §4, informed by doc 15 §8–§12/§151–§155.
...
class, status (doc 15 §10) and compatibility (§153); routing (ADR-D3-16), fallback
...
**Status rationale.** `Accepted` — registry is mandated by doc 15 §8; the
kill-switch is a governed refinement.

## 8. Architecture Detail

- **Schema** (doc 15 §15, §11–§12; doc 14 §15): `model_id`, `provider`, `version`,
...
- **Promotion** (doc 15 §151–§152, §155): status transitions ACTIVE⇄TESTING⇄
...
| RSK-02 | Stale registry vs deployed models | Low | Med | M | Startup validation; smoke test (doc 15 §82) | ML Eng | Low |
...
| Specification sections | doc 15 §8–§12, §56, §58, §83, §151–§155; doc 14 §15–§16 |
