...
intervening deterministic check is exactly what doc 2 §3.3 forbids.

## 2. Context and Problem Statement

Doc 1 §2.2 lists what the AI platform owns — nineteen items from conversation management to
AI observability. Doc 4 §4 lists runtime responsibilities per component. Doc 7 §4 names four core
components. Doc 3 §4's matrix marks capabilities as AI-owned. Doc 7 §67 and §69 draw a reasoning
boundary.
...
- Response generation uses a model and its output goes to a user — but only after passing the
  output guardrail.
- Guardrail evaluation must never use a model as its sole mechanism (doc 2 §3.3).
...
evaluation strategy incoherent: doc 21 requires AI quality evaluation, but unit-testing a
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | The nineteen AI-owned capabilities of doc 1 §2.2 must be classifiable | doc 1 §2.2 |
| DR-F-02 | The reasoning boundary must be expressible per capability | doc 7 §67, §69 |
| DR-F-03 | Critical controls must be deterministic | doc 2 §3.3 |
| DR-F-04 | Evaluation strategy must follow from capability class | doc 21 §1 |
...
| DR-N-03 | Class must determine controls, not merely describe | Each class names its required controls | doc 2 §3.3 |
...
| DR-C-02 | Model output is never authoritative | Platform | doc 3 §63; ADR-D1-03 |
| DR-C-03 | Every model interaction passes the harness | Platform | ADR-D2-09 §7.1 |
| DR-C-04 | The AI decides workflow and agent selection, context needs and communication | Organisational | doc 3 §46 |
...
| EC-03 | Evaluation strategy follows from class | 20 | Doc 21 requires evaluation; class should say what kind | Does class determine test versus evaluate? |
| EC-04 | Coverage of doc 1 §2.2's capabilities | 12 | All nineteen must classify | Unclassifiable capabilities |
...
- Maps directly to the package structure and to doc 1 §2.2's list.
...
- Says nothing about what controls apply — a guardrail that used a model and a response generator
  would classify identically, which is precisely the conflation doc 2 §3.3 warns against.
...
- Makes doc 2 §3.3 operational: Model-Terminal is the class that must not exist, and naming it
...
- Maps onto doc 20 §15's AI risk classification.
...
- Duplicates doc 20 §15–§17's risk classification, which already exists for a different purpose.
...
- Nothing makes doc 2 §3.3's prohibition checkable — the absence of a Model-Terminal class cannot
...
that matters. C's problem is that risk attaches to use rather than to capability, and doc 20 §15
...
Doc 2 §3.3's requirement that the SLM never be the sole enforcement mechanism, and doc 2 §48's
...
### 7.3 Doc 1 §2.2's capabilities, classified

| Capability (doc 1 §2.2) | Class | What constrains the model |
|---|---|---|
...
| RAG integration | C2 | Retrieval filters and ACL applied before retrieval (doc 13 §62) |
| Embedding / vector integration | C1 | — |
| SLM abstraction | C1 | The abstraction itself is deterministic |
| Prompt management / versioning | C1 | Deterministic composition (doc 16 §22) |
| Memory / session / cache | C1 | — |
| Input / output guardrails | **C1 — mandatory** | Must be deterministic per doc 2 §3.3 |
...
requirement, which is doc 2 §3.3 expressed as a classification.

Response generation is not in doc 1 §2.2's list because it is part of agent execution; it is the
platform's principal **C3** capability and carries C3's full control set.

### 7.4 Class determines evaluation, not just controls

Doc 21 requires AI quality evaluation, and the class says what kind:
...
doc 2 §3.3's prohibition is maintained rather than merely established.

**Status rationale.** Accepted. Tier 3 under ADR-D0-03 §7.1 — a classification framework rather
than a doc 2 §52 architecture change — ratified by the AI Solution Architect, with the Security
...
- Doc 2 §3.3's prohibition becomes an assertable property — "no C4 capability exists" — rather
  than a principle.
...
- Does not replace doc 20 §15's risk classification, which answers a governance question.
- Component inventories in doc 1 §2.2 and doc 4 §4 remain; this is an orthogonal cut.
...
| AC-02 | Every capability in doc 1 §2.2 has a recorded class | Audit against §7.3 |
...
| Specification sections | doc 1 §2.2 (AI-platform-owned); doc 3 §4 (Executive Responsibility Matrix), §64 (Ownership of AI Reasoning); doc 4 §4 (Runtime Responsibilities); doc 7 §4 (Four Core Components), §67 (Reasoning Boundary), §69 (AI Reasoning Boundary); doc 2 §3.3 (Deterministic Control), §48; doc 21 §1 |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Four classes by model trust and downstream check rather than by technology; C4 (Model-Terminal) named specifically so its absence is assertable, making doc 2 §3.3's prohibition testable; class determines both controls and evaluation strategy. |
