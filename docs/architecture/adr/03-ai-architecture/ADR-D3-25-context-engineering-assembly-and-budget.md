...
when large (doc 16 §123–§131; doc 15 §28–§32). Assembly order encodes the
...
Doc 16 §123–§124 define prompt context budget and budget strategy; §125–§131 define
composition with ERC batching, RAG, memory, API/tool results and authorization;
§138 covers source authority. Doc 15 §28–§32 define context budget, dynamic budget,
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Deterministic assembly order | doc 16 §20, §22 |
| DR-F-02 | Per-source token budget | doc 16 §123–§124; doc 15 §28–§29 |
| DR-C-01 | Assembly honours precedence (drop lowest authority first) | CLAUDE.md; doc 16 §138; ADR-D1-03 |
| DR-C-02 | ERC large → summarise/overflow safely | doc 15 §30–§32 |
| DR-C-03 | Trust tiers preserved (untrusted delimited) | ADR-D3-12; doc 16 §57 |
...
*lowest-authority* end (retrieved first, then memory), summarise large ERC (doc 15
...
|---|---|
| No budget (rely on window being big enough) | Overflow inevitable at large ERC (doc 15 §30) |
| Drop instructions to fit data | DR-C-01/EC-04 — safety violation |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 16 §123–§131/§138 and doc 15
...
platform trims from the lowest-authority end and summarises large ERC per doc 15 §31,
...
**Status rationale.** `Accepted` — doc 16 §123–§131 and doc 15 §28–§32 govern this;
ADR records the rationale.

## 8. Architecture Detail

- **Pipeline** `src/pf_ft_ai/context/` + prompt composer (ADR-D3-09): ordered stages
  each with a `token_budget`; a tokenizer-aware counter (doc 15 §27; doc 16 §122)
  enforces budgets.
...
- **ERC handling** (doc 15 §30–§32): large ERC is paginated/batched (MAX_ERC_BATCH_SIZE
...
| Audit and evidential requirements | Assembly recorded (prompt snapshot, doc 16 §89) |
...
| Specification sections | doc 16 §20, §73, §123–§131, §138; doc 15 §28–§32 |
| Requirement IDs | CTX-* |
| Build phases | 6, 8 |
