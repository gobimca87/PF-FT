...
Doc 7 §13 gives three confidence bands, with low and medium leading toward clarification. Doc 7 §14
gives the rule — the supervisor should not guess when the wrong workflow could trigger an incorrect
enterprise operation — with a registration example. Doc 4 §14 places the clarification path in the
runtime. Doc 10 §45 and §48 cover idempotency and unknown transaction state, which bear on what
"incorrect enterprise operation" costs. `SampleWorkflowchat.md` shows Adam offering explicit
choices at decision points.

Doc 7 §14's rule is sound and incomplete. It says *when* not to guess. It does not say:
...
user corrects. Doc 7 §13's bands would push a medium-confidence case toward clarification, and
...
**How many times to ask.** Doc 7 §14 permits clarification without bounding it. An assistant that
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Do not guess where a wrong workflow could trigger an incorrect enterprise operation | doc 7 §14 |
| DR-F-02 | Confidence bands drive routing behaviour | doc 7 §13; ADR-D2-05 §7.4 |
| DR-F-03 | The user must know what is being asked and why | `CLAUDE.md` persona rule 3 |
| DR-F-04 | State-changing operations must be intentional | doc 10 §45, §48 |
...
|---|---|---|---|---|
| EC-01 | Prevention of unintended irreversible operations | 35 | Doc 7 §14's concern; a wrongly submitted affiliation needs county intervention to undo | Can an irreversible operation occur without explicit intent? |
...
**Description.** Doc 7 §13's bands drive it: below the routing threshold, ask. No distinction
between reversible and irreversible consequences.

**Strengths.**
- Directly implements doc 7 §13 (EC-05).
...
- Doc 7 §14 is explicit that the supervisor should not guess where the wrong workflow could
...
Doc 7 §14 permits clarification without bounding it. The bound:
...
- Doc 7 §13's bands remain, as an input to clarification rather than its trigger.
...
| AC-04 | An ambiguity between workflows clarifies rather than proceeding | Golden case per doc 7 §14 |
...
| Specification sections | doc 7 §13 (Supervisor Confidence), §14 (Supervisor Clarification); doc 4 §14 (Clarification Path); doc 10 §45 (Idempotency), §48 (Unknown Transaction State); `Examples/SampleWorkflowchat.md`; `CLAUDE.md` persona rules 3, 6 |
