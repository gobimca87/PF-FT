...
class in config (doc 15 §33–§36), never hard-coded, and never used to compensate for
missing structure or validation.

## 2. Context and Problem Statement

Doc 15 §33–§36 define output-token budget and a temperature strategy; §37–§40 tie
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Deterministic tasks must be reproducible | doc 15 §36, §38 |
| DR-F-02 | Persona narration must feel natural | CLAUDE.md §Adam; ADR-D3-10 |
| DR-F-03 | Output-token budget enforced per workflow | doc 15 §33–§34 |
| DR-N-01 | Parameters declared, versioned, not hard-coded | doc 15 §35; CLAUDE.md |
| DR-C-01 | Temperature never substitutes for validation | doc 15 §39–§40 |
...
**Method.** Weighted scoring against §4, informed by doc 15 §33–§40.
...
output-token budgets are set per workflow (doc 15 §34). Parameters are never
hard-coded, and temperature is never raised to mask missing validation or structure
(doc 15 §39–§40). B/C/D rejected.

**Status rationale.** `Accepted` — doc 15 §33–§40 govern this; ADR records the
rationale.
...
| Specification sections | doc 15 §33–§40 |
