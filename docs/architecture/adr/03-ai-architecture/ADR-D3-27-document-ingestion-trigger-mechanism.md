...
source_docs:
  - "MD files/4 AI/13.PFF-FA-AI-RAG.md §11, §12, §13, §14, §15, §16, §17, §100"
build_phases: [8]
impacted_paths:
...
(`document updated` / `policy updated` / `source changed` — 13.PFF-FA-AI-RAG.md §100); (2)
...
13.PFF-FA-AI-RAG.md §11 shows the sequence `Source → discovered → registered → identity assigned →
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Enterprise-source document changes must reach RAG | 13.PFF-FA-AI-RAG.md §100 |
| DR-F-02 | Platform-authored content ingested via a governed pipeline | ADR-D3-11, D5-06 |
| DR-C-01 | No fabricated freshness — a stale index must be detectable | 13.PFF-FA-AI-RAG.md §14; ADR-D4-03-style provenance |
...
`document updated`/`policy updated`/`source changed` event (13.PFF-FA-AI-RAG.md §100) consumed by
...
ADR-D5-06, ADR-D6-15).
**Strengths.** Matches 13.PFF-FA-AI-RAG.md §100 exactly for enterprise sources; git-native
...
hash/timestamp (13.PFF-FA-AI-RAG.md §14) and re-ingests on diff.
...
**Method.** Weighted scoring against §4, informed by 13.PFF-FA-AI-RAG.md §11–§17/§100 and the
...
**Status rationale.** `Accepted` — 13.PFF-FA-AI-RAG.md §100 fixes Service Bus as the event
...
- `src/pff_fa_ai/rag/ingestion/`: an idempotent Service Bus consumer (ADR-D2-16,
  ...
  ADR-D2-17); each event resolves to a `document_id` (13.PFF-FA-AI-RAG.md §12) and enqueues a
...
- Change detection within either path still uses content hash + source-modified
  timestamp (13.PFF-FA-AI-RAG.md §14) to avoid re-embedding unchanged content on a duplicate
  event.
...
- Ingestion state model and staged-publish (13.PFF-FA-AI-RAG.md §16–§17 — a failed document never
...
| RSK-02 | Duplicate events cause redundant re-ingestion | Med | Low | L | Idempotent consumer + content-hash short-circuit (ADR-D2-18, 13.PFF-FA-AI-RAG.md §14) | Backend Lead | Low |
| RSK-03 | CI-on-merge ingest fails silently | Low | Med | M | Staged publish — failure never exposes a partial index (13.PFF-FA-AI-RAG.md §17) | Backend Lead | Low |
...
|---|---|
| Build phases | 8 |
| Repository paths | `src/pff_fa_ai/rag/ingestion/` |
...
| Specification sections | 13.PFF-FA-AI-RAG.md §11–§17, §100 |
| Requirement IDs | RAG-INGEST-TRIGGER-* |
| Build phases | 8 |
| Code paths | `src/pff_fa_ai/rag/ingestion/` |
