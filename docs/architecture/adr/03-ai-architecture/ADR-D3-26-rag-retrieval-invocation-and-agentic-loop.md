...
invoked, retrieval **executes deterministically** inside the RAG service (doc 13
§106, §110). Iterative "agentic" retrieval is supported but **bounded to a fixed
`max_iterations`** (doc 13 §108–§109) to prevent retrieval loops and uncontrolled
...
within a conversation turn. Doc 13 §106 defines a `rag.search` tool contract; §107 sets
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | RAG exposed as a formal, harness-gated tool contract | doc 13 §106 |
| DR-F-02 | Tool limits enforced (query length, top_k, filters, expansions, rounds, tokens) | doc 13 §107 |
| DR-C-01 | Retrieval execution deterministic once invoked | doc 13 §110 |
| DR-F-03 | Agentic/iterative retrieval bounded by max_iterations | doc 13 §108–§109 |
...
**Description.** RAG is registered as a tool (doc 13 §106) callable either by
...
determines context isn't actually needed; doesn't match doc 13 §110's framing of RAG
...
not parsed text); injection-prone; violates doc 13 §110's determinism requirement.
...
|---|---|
| Unbounded agentic RAG (no iteration cap) | doc 13 §109 — explicit loop-limit requirement |
| RAG invoked directly by any node without harness gating | ADR-D3-04 — mandatory tool-validation boundary |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 13 §105–§115 and the tool-gate
model of ADR-D3-04.
...
input contract and limits of doc 13 §106–§107, invocable by deterministic supervisor
...
**Status rationale.** `Accepted` — doc 13 §106–§110 fix the tool-contract and
...
- `src/pf_ft_ai/rag/retrieval/tool.py`: registers `rag.search(query: str, filters:
  ...
  (doc 13 §107) enforced at the harness gate (ADR-D2-09, ADR-D3-04 gate 3 — semantic
...
- **Invocation path**: for intents the intent classifier (ADR-D3-06) resolves
  ...
  bespoke code path.
- **Agentic loop** (doc 13 §108–§109): after a retrieval round, an evidence-
...
| Specification sections | doc 13 §105–§115 |
