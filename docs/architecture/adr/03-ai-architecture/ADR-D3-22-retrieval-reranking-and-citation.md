...
(doc 13 §51–§71, §81–§92). Below the score threshold, the platform says it doesn't
...
Doc 13 §51–§54 define vector/keyword/hybrid retrieval and why hybrid; §63–§71 cover
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | High recall + precision on knowledge queries | doc 13 §53–§54, §65 |
| DR-F-02 | Exact-identifier retrieval | doc 13 §52 (keyword); ADR-D3-23 |
| DR-F-03 | Every claim cited or not made | doc 13 §81, §82, §88, §91 |
| DR-F-04 | Low-confidence/no-result handled honestly | doc 13 §69–§71 |
| DR-C-01 | ACL filtering before/at retrieval | doc 13 §61–§62; ADR-D6-12 |
...
| EC-05 | Latency | 12 | Within budget (doc 13 §88) | p95 retrieval |
...
**Description.** Vector + BM25 candidates, fused (doc 13 §64), reranked (§65–§67),
...
**Weaknesses.** Permits unsupported claims — violates doc 13 §82; the core risk.
**Cost / effort.** Low; unsafe.

### 5.5 Option E — Agentic multi-hop retrieval (query planning + iterative retrieval)

**Description.** Decompose queries, retrieve iteratively (doc 13 §108–§113).
...
**Method.** Weighted scoring against §4, informed by doc 13 §51–§92 and the corpus
...
claim maps to a cited chunk (doc 13 §83) and citations are validated (§91); below the
...
**Status rationale.** `Accepted` — doc 13 §51–§92 govern this; ADR records rationale.
...
| Specification sections | doc 13 §51–§71, §81–§92, §127–§135 |
