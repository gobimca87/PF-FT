...
mutable prompt. This mirrors doc 16 §169 ("prompt source of truth" = Git) and the
versioned-artefact rule in `CLAUDE.md`.

## 2. Context and Problem Statement

Doc 16 §31–§39 defines a template registry, file naming, metadata, status lifecycle
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Load prompt deterministically by id@version | doc 16 §35, §87 |
| DR-F-02 | Full review/diff/history on every prompt change | doc 16 §96, §157 |
| DR-F-03 | Promote across DEV→…→PROD as immutable release | doc 16 §102, §161 |
| DR-F-04 | Roll back to a prior version instantly | doc 16 §103, §160 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Production prompt is immutable | No runtime edit path | doc 16 §39, §155 |
| DR-N-02 | Provenance on every generation | Version in every trace | doc 16 §90 |
| DR-N-03 | Human-readable authoring + machine metadata | MD body + YAML | doc 16 §167, §168 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | Prompts are versioned artefacts, not in-place mutable | Organisational | CLAUDE.md; doc 16 §39 |
| DR-C-02 | Change governance/approval gates apply | Governance | ADR-D6-15; doc 16 §96 |
| DR-C-03 | Secrets never embedded in prompts | Security | doc 16 §114, §116 |
...
| EC-06 | Security (secret-free, scannable) | 10 | doc 16 §114 | Secret-scan in CI |
| | **Total** | **100** | | |
...
`id@version`; promotion pins versions in an immutable release manifest (doc 16 §161).
...
**Weaknesses.** Runtime mutability violates DR-C-01/doc 16 §39; weak diff/review;
...
prompt-engineering lifecycle (doc 16 §40); harder for prompt engineers; no metadata
...
**Weaknesses.** Off-tenancy storage of prompts (some sensitive, doc 16 §116); new
...
**Method.** Weighted scoring against §4, informed by doc 16 §166–§169 and the
...
**Status rationale.** `Accepted` — doc 16 §169 and CLAUDE.md fix Git-as-truth and
immutability; this ADR records the alternatives and rationale.

## 8. Architecture Detail

- **Layout** (doc 16 §84): `prompts/{system,persona,task,tool}/…vN.md`, each with
  metadata (doc 16 §33): `id`, `version` (semver, §35–§36), `status` (§34),
  `owner` (§40), `risk_class` (§41), `model_compatibility` (§82), `dependencies`
  (§106).
- **Registry/loader** (doc 16 §86–§87): resolves and caches by `id@version`;
  missing/invalid version fails closed (doc 16 §29).
- **Promotion** (doc 16 §102, §118 overlays, §161 manifest): environment overlays
  layer non-secret env values; the release manifest pins exact versions per env.
- **Rollback** (doc 16 §103, §160): change the manifest pointer; previous version is
  still present and immutable.
- **CI gates** (doc 16 §113 lint, §114 secret-scan, §155 regression): every prompt
...
|---|---|---|---|---|---|---|---|
| RSK-01 | Secret committed in a prompt | Low | High | M | CI secret-scan (doc 16 §114) | Security Architect | Low |
...
| Configuration | Env overlays (doc 16 §118); manifest (§161) |
...
| Specification sections | doc 16 §31–§41, §84–§87, §102–§103, §161, §166–§169 |
