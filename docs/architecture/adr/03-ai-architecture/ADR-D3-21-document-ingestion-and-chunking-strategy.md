...
source_docs:
  - "MD files/4 AI/13.PFF-FA-AI-RAG.md §8, §9, §10, §14, §15, §19, §22, §23, §24, §27, §28, §29, §30, §40, §41, §42, §43, §44, §45, §46, §47, §48"
  - "MD files/4 AI/14.PFF-FA-AI-EMBEDDING-VECTOR.md §20, §21, §23, §24"
build_phases: [8]
impacted_paths:
  - src/pff_fa_ai/rag/ingestion/
  - src/pff_fa_ai/rag/chunking/
...
13.PFF-FA-AI-RAG.md §40 requires semantic rather than blind N-character chunking. §41 states there is no
...
| Corpus size | Low hundreds of documents (FA and county policy, safeguarding guidance, affiliation handbooks, insurance product documentation) | 13.PFF-FA-AI-RAG.md §9 source types; affiliation flow Phase 0 |
...
| Document character | Long-form, heavily structured, authoritative policy | 13.PFF-FA-AI-RAG.md §10 `CONTROLLED_REFERENCE` and above |
...
13.PFF-FA-AI-RAG.md §44 requires a chunk to "retain sufficient context", and for cross-referencing policy text
...
chunks give the model enough to reason over. 13.PFF-FA-AI-RAG.md §45's parent-child addresses this and needs
a decision on how it composes with the chunking strategy.

**Heterogeneous formats need different treatment.** 13.PFF-FA-AI-RAG.md §43 lists five format families with
different structural signals; 13.PFF-FA-AI-RAG.md §19–§26 cover extraction per format including OCR (§20–§21)
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Chunking must be semantic/structural, not blind splitting | 13.PFF-FA-AI-RAG.md §40 |
| DR-F-02 | Chunk size must be evaluation-determined, not asserted | 13.PFF-FA-AI-RAG.md §41 |
| DR-F-03 | Strategy must vary by document type | 13.PFF-FA-AI-RAG.md §43 |
| DR-F-04 | Chunks must retain sufficient context and preserve identifiers | 13.PFF-FA-AI-RAG.md §44 |
| DR-F-05 | Parent-child retrieval must be supported | 13.PFF-FA-AI-RAG.md §45 |
| DR-F-06 | Chunk hashing must support change detection | 13.PFF-FA-AI-RAG.md §14, §48 |
| DR-F-07 | Chunk metadata must carry source, security and business fields | 13.PFF-FA-AI-RAG.md §30, §31, §32 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Retrieval quality must meet the evaluation threshold | Set at Phase 8 by measurement | 13.PFF-FA-AI-RAG.md §41 |
...
|---|---|---|---|
| DR-C-01 | RAG carries knowledge only, never operational truth | Platform | ADR-D3-20; 13.PFF-FA-AI-RAG.md §2 |
| DR-C-02 | Chunk metadata must support ACL filtering before retrieval | Platform | 13.PFF-FA-AI-RAG.md §33–§38, §62 |
| DR-C-03 | Embedding dimension is an architecture constraint | Platform | 14.PFF-FA-AI-EMBEDDING-VECTOR.md §18 |
...
| EC-02 | Context preservation across boundaries | 25 | 13.PFF-FA-AI-RAG.md §44; cross-referencing policy text is the corpus's defining characteristic | Does a retrieved chunk carry enough to be understood alone? |
| EC-03 | Format coverage | 15 | 13.PFF-FA-AI-RAG.md §43's five families | Formats handled without a separate pipeline |
...
ignoring structure. 13.PFF-FA-AI-RAG.md §42's baseline (400/50) applied uniformly.
...
- Directly contradicts 13.PFF-FA-AI-RAG.md §40's requirement for semantic chunking.
...
- Fails 13.PFF-FA-AI-RAG.md §44's context-retention criterion for cross-referencing text.
...
- Tables and lists still fragment (13.PFF-FA-AI-RAG.md §22's table extraction requirement unmet).
- 13.PFF-FA-AI-RAG.md §40's "rather than blindly splitting" is only partially satisfied.
...
subsection boundaries per 13.PFF-FA-AI-RAG.md §40's preferred hierarchy, splitting oversized sections at
paragraph boundaries.

**Strengths.**
- Chunks align with the author's own semantic units, which for policy documents is exactly right.
- Directly implements 13.PFF-FA-AI-RAG.md §40 and §43.
- Preserves tables and lists intact (13.PFF-FA-AI-RAG.md §22).
...
- **Does not solve cross-referencing.** A chunk from section 7 still does not know section 4.2
  exempts youth teams.
- Per-format extractors to build and maintain (13.PFF-FA-AI-RAG.md §19–§26).
...
(13.PFF-FA-AI-RAG.md §46–§47): for each chunk, an LLM call generates a short prefix situating it in its
document — what the document is, which section this is, what it depends on — which is prepended
before embedding. **Parent-child** (13.PFF-FA-AI-RAG.md §45): the small chunk is indexed and retrieved; its
...
- Retrieval precision from small chunks; generation context from parent sections (13.PFF-FA-AI-RAG.md §45).
...
- Non-deterministic: the same document chunks differently across runs, so re-ingest changes chunk
  identity and breaks 13.PFF-FA-AI-RAG.md §48's chunk hashing and change detection.
...
| Sentence-level chunking | Fragments policy clauses below the level at which they carry meaning; fails 13.PFF-FA-AI-RAG.md §44 |
...
| Mechanism | 13.PFF-FA-AI-RAG.md | What it does |
|---|---|---|
...
Per 13.PFF-FA-AI-RAG.md §43 and §19–§26:

| Format | Structural signal | 13.PFF-FA-AI-RAG.md | Fallback when absent |
|---|---|---|---|
...
Per 13.PFF-FA-AI-RAG.md §41 and DR-F-02, no chunk size is fixed here. 13.PFF-FA-AI-RAG.md §42's 400/50 is the **starting
...
Per 13.PFF-FA-AI-RAG.md §14 and §48:
...
```mermaid
flowchart TD
    S[Registered source<br/>13.PFF-FA-AI-RAG.md §8] --> D[Document registration<br/>§11, identity §12, version §13]
...
- Per-format extractors are real work (13.PFF-FA-AI-RAG.md §19–§26), including OCR for scanned PDFs.
- The recommendation is churn-dependent and must be revisited if the corpus profile changes.

### 9.3 Neutral

- 13.PFF-FA-AI-RAG.md §42's 400/50 baseline is a starting point for evaluation, as §42 itself says.
...
| RSK-06 | OCR errors propagate into chunks and prefixes | Medium | Medium | Medium | 13.PFF-FA-AI-RAG.md §21's OCR confidence recorded; low-confidence documents flagged for review before indexing | RAG/Data Owner | Medium |
...
| Dimension | Impact |
|---|---|
| Attack surface change | Documents entering the index are untrusted content (ADR-D3-09 T3). File security applies at ingest (13.PFF-FA-AI-RAG.md §18). Contextual augmentation adds an LLM call over document content, so a document containing injected instructions could influence its own prefix — §7.4 rule 4's validation is the mitigation. |
| Data classification touched | Policy and guidance documents, classified per 13.PFF-FA-AI-RAG.md §39. Security metadata attaches per §32. |
| Personal data / PII | Policy documents should not contain personal data. Where a source does (a named contact in a handbook), 13.PFF-FA-AI-RAG.md §32's security metadata and ACL filtering apply, and the RAG/Data Owner assesses at source registration (13.PFF-FA-AI-RAG.md §8). |
...
| Repository paths | `src/pff_fa_ai/rag/ingestion/`, `src/pff_fa_ai/rag/chunking/` |
| Configuration | Chunking parameters, augmentation prompt reference and per-format strategy in `config/base/` |
| Contracts / schemas | Chunk model with metadata per 13.PFF-FA-AI-RAG.md §29–§32; parent-child linkage |
...
| Failure mode and degradation | A chunk failing prefix validation is not indexed and is reported (13.PFF-FA-AI-RAG.md §17's ingestion failure state). The document's other chunks index normally, so a partial failure degrades coverage rather than blocking ingest. |
...
| RT-06 | 13.PFF-FA-AI-RAG.md §40–§48 amended | Change notice | Re-derive the strategy against the revised requirements |
...
| Specification sections | 13.PFF-FA-AI-RAG.md §8 (Source Registry), §9 (Supported Source Types), §10 (Source Authority), §11–§14 (Registration, Identity, Version, Change Detection), §15 (Ingestion Pipeline), §17 (Ingestion Failure), §18 (File Security), §19–§26 (Extraction, OCR, OCR Confidence, Table, Excel, PDF, Word, PowerPoint), §27 (Content Normalization), §28–§32 (Metadata Architecture, Required, Chunk, Business, Security), §39 (Document Classification), §40 (Chunking Strategy), §41 (Chunk Size), §42 (Chunk Overlap), §43 (Chunking by Document Type), §44 (Chunk Quality), §45 (Parent-Child Retrieval), §46 (Contextual Chunking), §47 (Contextual Retrieval), §48 (Chunk Hash); 14.PFF-FA-AI-EMBEDDING-VECTOR.md §20–§21 (Chunk-to-Vector Mapping, Chunk Identity), §23–§24 (Chunk Content Storage, Recommended Separation) |
| Requirement IDs | `FR-A39-08`, `NFR-A38-PERF`, `NFR-A38-COST` |
| Build phases | 8 |
| Code paths | `src/pff_fa_ai/rag/ingestion/`, `src/pff_fa_ai/rag/chunking/` |
