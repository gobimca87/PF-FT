...
Doc 15 §5 defines "initial and future architecture" and §19–§21 describe the HF
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Generate language for orchestration/communication (never business truth) | doc 15 §2, §4, §40 |
| DR-F-02 | Provider-neutral access; swap providers without code change | doc 15 §6, §15; ADR-D3-14 |
| DR-F-03 | Support fallback across providers/models | doc 15 §62; ADR-D3-18 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | Time-to-first-value | Weeks, not GPU-platform months | doc 15 §19 |
| DR-N-02 | Data boundary control at scale | In-tenancy inference for sensitive flows | doc 15 §124–§126; ADR-D6-07 |
| DR-N-03 | Unit cost at volume | Self-host cheaper per token at scale | doc 15 §103–§105 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | Direction fixed: HF first → self-hosted target | Organisational | CLAUDE.md; doc 15 §5 |
| DR-C-02 | SLM never executes business rules / makes authz | Regulatory/Arch | doc 15 §4, §40 |
| DR-C-03 | Access only via provider abstraction | Architecture | doc 15 §6, §15; ADR-D3-14 |
| DR-C-04 | Sensitive data minimised before external inference | Security | doc 15 §125, §126; ADR-D6-07 |

### 3.4 Assumptions

| ID | Assumption | If false | Validation |
|---|---|---|---|
| DR-A-01 | An open SLM meets quality on PF-FT tasks | Consider larger/commercial model behind abstraction | SLM eval (doc 15 §92) |
| DR-A-02 | Volume grows enough to justify self-host | Stay on HF longer | Cost model (doc 15 §106) |
...
| EC-03 | Quality on PF-FT tasks | 18 | Must be good enough | Eval scores (doc 15 §97) |
| EC-04 | Unit cost at target volume | 15 | Long-run economics | £/1k workflows (doc 15 §106) |
...
EC-02 = 22 is justified by the safeguarding/PII sensitivity of FA data (doc 15
...
premature capacity/cost commitment (doc 15 §85 warns against premature complexity).
...
**Description.** Route by task class (doc 15 §56–§60): self-host handles routine
...
| Client-side / on-device model | Enterprise server-side architecture (doc 15 §3) |

## 6. Evaluation Method and Decision Matrix

**Method.** Weighted scoring against §4, informed by doc 15 §5, §19–§21, §85,
...
evaluation (doc 15 §92, §99), cost (§106) and data-boundary review (ADR-D6-07). The
...
**Status rationale.** `Accepted` — the direction is fixed by CLAUDE.md and doc 15
...
- **Abstraction** (doc 15 §6, §15; ADR-D3-14): `SLMProvider` protocol with
...
- **Config/secrets** (doc 15 §16–§18): endpoints and `*_secret_ref` via Key Vault
  (ADR-D5-07); per-environment config (§17).
- **Data minimisation** (doc 15 §125–§126; ADR-D6-07): sensitive fields minimised/
  redacted before any external inference call; safeguarding-sensitive flows
  prioritised for early self-host cutover.
- **Fallback** (doc 15 §62–§64; ADR-D3-18): fallback must not be silent; abstraction
...
|---|---|
| Enterprise decides; AI orchestrates | SLM generates language only; never business truth/authz (doc 15 §4, §40) |
...
| Versioned artefacts | Model + config versioned (ADR-D3-15; doc 15 §154–§155) |
...
|---|---|---|---|---|---|
| QM-01 | SLM eval score on PF-FT tasks | ≥ gate | < gate | Eval (doc 15 §97, §99) | Every model release |
...
| Personal data / PII | Minimised/redacted pre-inference (doc 15 §126) |
...
| AC-04 | Fallback is logged, never silent | Test (doc 15 §64) |
...
| Failure mode and degradation | Provider down → fallback → degraded mode (doc 15 §163, §168) |
...
|---|---|---|---|
| HF Inference API | setup | usage-based | doc 15 §104 |
| Self-hosted GPU (later) | platform build | GPU node hours | doc 15 §105; ADR-D5-11 |
...
| Specification sections | doc 15 §2–§5, §19–§21, §85, §103–§106, §124–§126 |
