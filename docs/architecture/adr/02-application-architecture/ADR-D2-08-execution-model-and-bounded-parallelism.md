...
source_docs:
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §31, §32, §33, §34"
  - "MD files/3 Context & Integration/10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §50, §51, §52, §53, §54, §55, §56, §57"
  - "MD files/1 Foundation/1 PFF-FA-AI-ARCHITECTURE.md §13"
  - "MD files/1 Foundation/4. PFF-FA-AI-RUNTIME.md §23, §24, §57, §58"
build_phases: [5, 6]
impacted_paths:
  - src/pff_fa_ai/integration/execution/
  - src/pff_fa_ai/context/collection/
...
1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 4 requires sequential and parallel AI execution. 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §31–§34 cover
sequential and parallel API execution, parallel execution controls, and fan-out/fan-in. 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md
...
**Who decides execution order?** 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §53 names an execution planner. It does not say what the
...
**What bounds parallelism, and why?** 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §55 requires bounded parallelism without saying what
the bound protects. This matters: a bound sized to protect the platform's event loop is a
different number from one sized to protect an enterprise service's connection pool. 25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §64
...
**What happens when one parallel branch fails?** 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §56 covers parallel failure handling and
8 PFF-FA-AI-ERC-CONTEXT.md §49–§50 distinguish mandatory from optional collection failure. The distinction is
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Sequential, parallel and hybrid execution must all be supported | 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 4; 1 PFF-FA-AI-ARCHITECTURE.md §13 |
| DR-F-02 | An execution planner must derive the order | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §53 |
| DR-F-03 | API dependencies must be declared | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §52; 8 PFF-FA-AI-ERC-CONTEXT.md §25 |
| DR-F-04 | Parallelism must be bounded | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §55; 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §33 |
| DR-F-05 | Mandatory and optional context must be distinguished | 8 PFF-FA-AI-ERC-CONTEXT.md §24, §49, §50 |
| DR-F-06 | Fan-out/fan-in must be supported for collections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §34; 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §54 |
...
| DR-N-02 | Enterprise services must not be overloaded by fan-out | Within published rate limits | 25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §64 |
| DR-N-03 | Execution order must be deterministic and testable | Same requirements, same plan | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §22 |
...
| DR-C-02 | ERC batch size is 20 | Platform | 8 PFF-FA-AI-ERC-CONTEXT.md §36; ADR-D4-04 |
| DR-C-03 | The model must not decide execution order | Platform | ADR-D1-02 §7.1 I-6; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.3 |
| DR-C-04 | Enterprise APIs declare whether they are parallelisable | Platform | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §10 |
...
**Weaknesses.**
- Fails 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 4, which requires parallel execution.
...
- Violates DR-C-03 and 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.3: this is a critical control, and the SLM must not be the only
...
parallelisable (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §10). The execution planner topologically sorts the required calls into
...
- Mandatory/optional decided before execution, so partial failure resolves without judgement
  (EC-05).
- Directly implements 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §52–§56.
...
Each API in the catalogue declares, per 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §10:
...
The execution planner (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §53):

1. Takes the context requirements for the current workflow step (8 PFF-FA-AI-ERC-CONTEXT.md §22–§23).
2. Resolves each to its catalogue entry.
3. Builds the dependency graph (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §52).
...
This is hybrid execution as 1 PFF-FA-AI-ARCHITECTURE.md §13 describes it: sequential between stages, parallel within
...
- **No placeholder substitution.** The planner never substitutes a default, empty or synthesised
  identifier to make a call executable. 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §35's input validation rejects it.
...
10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §55 requires bounded parallelism without stating what the bound is for. It is stated here
...
> from enterprise capacity and rate limits (25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §64), not from the platform's own throughput.
...
- The bound is *not* a platform protection mechanism. Platform-side protection — connection pool
  limits, event loop health — is ADR-D5-16's and 4. PFF-FA-AI-RUNTIME.md §57–§58's concern, and uses separate
  controls.
...
Per 8 PFF-FA-AI-ERC-CONTEXT.md §24, every context requirement declares whether it is mandatory or optional for the
current workflow step. The classification is **in the workflow definition**, not decided after a
failure.

| Classification | On failure |
|---|---|
| **Mandatory** | The step cannot proceed. Retry per policy (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §41–§44); on exhaustion, the workflow reports it cannot continue and states what is unavailable (8 PFF-FA-AI-ERC-CONTEXT.md §49). |
| **Optional** | Execution continues. ERC records the section as incomplete with a reason (8 PFF-FA-AI-ERC-CONTEXT.md §50–§51). Downstream reasoning is told the section is absent, not given a silent gap. |
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §34 and 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §54 require fan-out/fan-in. Applied to collections, it composes with
batching (ADR-D4-04):

1. A collection is partitioned into batches of at most 20 (8 PFF-FA-AI-ERC-CONTEXT.md §36, DR-C-02).
2. Batches fan out under the §7.3 per-service bound.
3. Results fan in and are aggregated in a deterministic order (8 PFF-FA-AI-ERC-CONTEXT.md §58).
4. Per-batch failures are handled per 8 PFF-FA-AI-ERC-CONTEXT.md §48's partial batch failure rules.
...
10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §56 and 8 PFF-FA-AI-ERC-CONTEXT.md §48–§50 combine:
...
3. Optional failures are recorded in ERC completeness tracking (8 PFF-FA-AI-ERC-CONTEXT.md §51–§52).
4. Dependent stages are not executed if a mandatory prerequisite failed (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §57).
...
- Hybrid execution is the specification's own model (1 PFF-FA-AI-ARCHITECTURE.md §13); this decision fixes how the plan
  is produced.
...
| RSK-05 | Aggregation ordering non-deterministic, producing varying prompt content | Low | Medium | Low | Deterministic ordering per 8 PFF-FA-AI-ERC-CONTEXT.md §58; AC-06 | AI Engineering Lead | Low |
...
| Repository paths | `src/pff_fa_ai/integration/execution/planner.py`, `dependency.py`, `concurrency.py`; `src/pff_fa_ai/context/collection/` |
...
| AC-04 | A mandatory failure blocks the step; an optional failure records incompleteness | Failure injection per 8 PFF-FA-AI-ERC-CONTEXT.md §49–§50 |
...
| Specification sections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §31 (Sequential API Execution), §32 (Parallel API Execution), §33 (Parallel Execution Controls), §34 (Fan-Out/Fan-In); 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §50–§57 (Sequential, Parallel, Dependency Graph, Execution Planner, Fan-Out/Fan-In, Bounded Parallelism, Parallel Failure Handling, Tool Dependency Failure), §10 (Extended Metadata), §35 (Tool Input Validation); 8 PFF-FA-AI-ERC-CONTEXT.md §22–§25 (Context Requirements, Dependency Graph), §36 (Agreed Batch Size), §48–§52 (Partial Batch Failure, Mandatory/Optional Failure, Completeness), §58 (Aggregation Ordering); 1 PFF-FA-AI-ARCHITECTURE.md §13, §39 criterion 4; 4. PFF-FA-AI-RUNTIME.md §23–§24, §57–§58; 25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §64 |
| Requirement IDs | `FR-A39-04`, `FR-A39-05`, `NFR-A38-PERF`, `NFR-A38-SCALE` |
| Build phases | 5, 6 |
| Code paths | `src/pff_fa_ai/integration/execution/`, `src/pff_fa_ai/context/collection/` |
