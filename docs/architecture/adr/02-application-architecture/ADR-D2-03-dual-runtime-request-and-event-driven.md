...
Doc 1 §4 and §5 describe a request-driven runtime and an event-driven runtime as two runtimes.
Doc 4 §5 gives the request lifecycle across twenty-odd stages; doc 4 §47–§49 give the Service Bus
runtime, event-to-workflow mapping and HIL runtime. Doc 11 §32–§34 give consumer architecture and
responsibilities.
...
There is a second, subtler problem that doc 11 §58 names precisely: *"Event Should Not Become
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Events must refresh ERC and resume workflows | doc 1 §39 criterion 12; doc 11 §50, §55 |
| DR-F-02 | An event is a notification, not a data source or an instruction | doc 11 §25, §58 |
| DR-F-03 | Guardrails apply at every boundary, on both paths | doc 18; ADR-D1-02 |
| DR-F-04 | Long-running workflows survive request termination | doc 1 §39 criterion 13; doc 4 §46 |
| DR-F-05 | Event processing must be idempotent | doc 11 §40–§44 |
| DR-F-06 | The consumer must not make business decisions | doc 11 §34 |
...
| DR-N-02 | Event processing must not block on a user | 0 user-waiting dependencies | doc 11 §5 |
...
|---|---|---|---|
| DR-C-01 | Events carry no user authorization; the emitting system is not a user | Platform | doc 3 §5.2; doc 11 §23 |
| DR-C-02 | Event payloads are notifications; authoritative data comes from a subsequent API read | Platform | doc 11 §25, §26 |
...
| DR-A-02 | Enterprise events are reliably delivered, if not reliably ordered | Reconciliation becomes the primary mechanism rather than the backstop | doc 11 §45–§48; ADR-D2-18 |
...
- Matches doc 4's structure: §5's request lifecycle and §47's Service Bus runtime converge at
  the harness.
...
- Contradicts doc 4 §5's synchronous request lifecycle.
...
| Identity | APIM-validated claims (doc 3 §5.2) | **None** — see §7.3 |
| Correlation | Generated or continued from the request | Carried on the event envelope (doc 11 §19) |
| Deadline | HTTP timeout, user waiting | Message lock renewal, no user waiting |
| Idempotency | Conversation turn semantics | Event ID deduplication (doc 11 §40–§44) |
...
Doc 11 §25 and §26 establish the pattern; doc 11 §58 adds the injection prohibition. Both are
adopted as binding:

- **Not data.** An event says *something changed*. Authoritative values come from a subsequent
  enterprise API read (doc 11 §26's event-to-API refresh pattern). An event payload's field
...
- **Not an instruction.** Event payload content is never composed into a prompt as
  ...
  mapping (doc 4 §48); its payload provides identifiers for the refresh. No free text from an
  event reaches the model as directive content.

The second rule is what closes the injection channel doc 11 §58 identifies. It is enforced
...
1. Envelope validated; schema version checked (doc 11 §37–§38).
2. Deduplicated by event ID (doc 11 §41).
3. Workflow instance resolved from `workflow_instance_id` (doc 11 §21).
...
- The event-payload-to-prompt injection channel doc 11 §58 warns about is closed structurally.
...
- Doc 1 §4 and §5's "two runtimes" is realised as two entries to one runtime, which is a
...
| Versioned artefacts, never mutated in place | Event contracts are versioned (doc 11 §16); unknown versions handled per doc 11 §38. |
...
| RSK-06 | Out-of-order or stale events advance workflow state incorrectly | Medium | Medium | Medium | Sequence and staleness handling per doc 11 §45–§47; reconciliation per ADR-D2-18 | AI Engineering Lead | Medium |
...
| Dimension | Impact |
|---|---|
| Attack surface change | The Service Bus subscription is an ingress point. §7.4's structural prohibition on payload-to-prompt paths closes the injection channel doc 11 §58 identifies; envelope and schema validation (doc 11 §36–§37) close malformed-input paths. |
| Data classification touched | Event payloads carry identifiers, not personal data by design (doc 11 §24). Refreshed API responses carry the personal data. |
...
| Audit and evidential requirements | Correlation and causation IDs (doc 11 §19–§20) link event to workflow to refresh to eventual user notification, giving an unbroken chain. |
...
| Configuration | Event-type-to-handler registry; subscription filters (doc 11 §30) |
| Contracts / schemas | Event envelope (doc 11 §13); `ExecutionContext` model |
| Migration | None |
...
| RT-05 | Event ordering problems exceed reconciliation's capacity (DR-A-02) | Incident records | Strengthen sequence handling per doc 11 §45–§46 |
