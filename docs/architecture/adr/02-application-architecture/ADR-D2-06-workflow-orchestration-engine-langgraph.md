...
source_docs:
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §9, §25, §28, §29, §30"
  - "MD files/1 Foundation/1 PFF-FA-AI-ARCHITECTURE.md §12, §13, §39"
  - "MD files/1 Foundation/2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §11, §13, §28"
build_phases: [4]
impacted_paths:
  - src/pff_fa_ai/orchestration/langgraph/
...
`CLAUDE.md` lists LangGraph under Confirmed Tech Stack. 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §9 assigns it responsibility, §25
gives an example `AffiliationGraph`, §28 lists twenty typical nodes, §29 classifies nodes as
deterministic or AI, §30 gives edge types. 1 PFF-FA-AI-ARCHITECTURE.md §12 and §13 cover LangGraph and
sequential/parallel/hybrid execution. 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 4 requires that *"LangGraph supports
...
The choice is consequential because of what the platform's requirements actually are. 1 PFF-FA-AI-ARCHITECTURE.md §39
criterion 13 requires long-running workflows to survive request termination. 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §29 requires
HIL suspension across days. 11 PFF-FA-AI-SERVICE-BUS.md §55 requires resume driven by Service Bus events. ADR-D2-03
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Sequential, parallel and hybrid AI execution must be supported | 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 4; 1 PFF-FA-AI-ARCHITECTURE.md §13 |
| DR-F-02 | Long-running workflows must survive request termination | 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 13; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §28 |
| DR-F-03 | Nodes must be classifiable as deterministic or AI | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §29 |
| DR-F-04 | Graph state must be strongly typed and reference-based | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26–§27; ADR-D2-07 |
| DR-F-05 | Resume must run under a captured, revalidated authorization context | ADR-D2-03 §7.3 |
| DR-F-06 | Each agent may have its own graph | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §25 |
...
|---|---|---|---|
| DR-A-01 | LangGraph's graph model expresses the node and edge shapes 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §28–§30 require | Some workflow shapes need bespoke traversal | Phase 4 spike against the affiliation graph |
...
|---|---|---|---|---|
| EC-01 | Fitness for the platform's actual durability and HIL requirements | 30 | 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 13 and ADR-D2-03 §7.3 are unusual requirements; a mechanism that half-fits is worse than none | Does the mechanism satisfy captured-context revalidation and multi-day, cross-workload resume? |
...
- Framework retry policies would compete with the platform's own retry hierarchy (4. PFF-FA-AI-RUNTIME.md §56) and
  its idempotency model (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47), producing two retry layers with different semantics.
...
- A bespoke engine forfeits LangGraph's genuine strength: expressing conditional and parallel
  graph traversal declaratively, which 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §30 and 1 PFF-FA-AI-ARCHITECTURE.md §13 both require.
- Recorded to show the confirmed choice is defensible: LangGraph's declarative graph model is a
  good fit for 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §25–§30's node and edge shapes, which is the part the platform actually
  needs.
...
- declaring edges, including conditional edges (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §30);
- fan-out and fan-in for parallel branches (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §34; 1 PFF-FA-AI-ARCHITECTURE.md §13);
...
| Retry and timeout | ADR-D2-11; 4. PFF-FA-AI-RUNTIME.md §55–§56 | One hierarchy; framework retries would compete with tool-level and API-level retry and with the idempotency model |
| Idempotency | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47 | Tied to enterprise operation semantics, not graph traversal |
...
`langgraph` may be imported **only** by modules under `src/pff_fa_ai/orchestration/langgraph/`.
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §29's deterministic/AI classification is adopted and made structural. Every node declares
its class:

| Class | Examples (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §28) | Property |
|---|---|---|
...
Per 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §25, each agent declares its own graph. `AffiliationGraph` is built from 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §28's
node vocabulary, using the subset affiliation needs — 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §28 notes explicitly that *"not every
...
**Status rationale.** Accepted. Tier 1 under ADR-D0-03 §7.1 — LangGraph architecture is a 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md
...
- LangGraph remains the confirmed engine per `CLAUDE.md`; this decision sets its depth, not its
  identity.
- 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §25–§30's node and edge model is adopted essentially unchanged.
...
| RSK-06 | Graph state grows large, defeating 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27's reference rule | Medium | Medium | Medium | ADR-D2-07's reference-only rule with a size assertion in tests | AI Engineering Lead | Low |
...
| Data classification touched | Graph state references personal data in ERC rather than carrying it, per 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 — which limits what is serialised at suspension. |
...
| Repository paths | `src/pff_fa_ai/orchestration/langgraph/` — builder, state, node registry |
...
| Specification sections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §9 (LangGraph Responsibility), §25 (LangGraph Graph Model), §26–§27 (Graph State, Graph State Rule), §28 (Graph Nodes), §29 (Deterministic vs AI Nodes), §30 (Graph Edge Types), §34 (Fan-Out/Fan-In); 1 PFF-FA-AI-ARCHITECTURE.md §12 (LangGraph), §13 (Sequential/Parallel/Hybrid), §39 criteria 4, 13; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §11 (LangGraph Execution Architecture), §13 (Execution Pattern), §28 (Durable Workflow Architecture); 4. PFF-FA-AI-RUNTIME.md §19–§20, §55–§56; `CLAUDE.md` |
| Requirement IDs | `FR-A39-04`, `FR-A39-13`, `NFR-A38-MAINT` |
| Build phases | 4 |
| Code paths | `src/pff_fa_ai/orchestration/langgraph/` |
