...
source_docs:
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8, §50, §61, §67, §68, §69, §72, §73"
  - "MD files/1 Foundation/1 PFF-FA-AI-ARCHITECTURE.md §11"
  - "MD files/1 Foundation/2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §10"
  - "MD files/1 Foundation/4. PFF-FA-AI-RUNTIME.md §16"
build_phases: [4]
impacted_paths:
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8 calls the harness *"the controlled runtime boundary"* and lists seventeen things it
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50 gives an eleven-stage execution pipeline. 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §10 and 1 PFF-FA-AI-ARCHITECTURE.md §11 give it a place.
...
**Does it apply per node, per agent run, or per turn?** 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50's pipeline reads as a
per-invocation sequence, but 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72's agent loop and §73's loop protection imply an agent
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | The harness must prevent the SLM controlling unrestricted runtime operations | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8 |
| DR-F-02 | The eleven-stage pipeline must execute for agent work | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50 |
| DR-F-03 | Loop protection and limits must be enforced | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72–§73; 4. PFF-FA-AI-RUNTIME.md §54 |
| DR-F-04 | Retries and timeouts follow a single hierarchy | 4. PFF-FA-AI-RUNTIME.md §55–§56 |
...
| DR-N-02 | An agent must not be able to bypass the harness | Structurally impossible, not merely prohibited | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8 |
| DR-N-03 | Agents must remain testable without the full harness | Unit-testable node handlers | 22.PFF-FA-AI-TESTING.md |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | Critical controls are deterministic; the SLM is never the sole mechanism | Platform | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.3 |
| DR-C-02 | Agents may not bypass tools, override authorization or execute unrestricted APIs | Platform | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §5 |
| DR-C-03 | Layering: orchestration imports infrastructure interfaces only | Platform | ADR-D2-01 §7.2 |
| DR-C-04 | Guardrails apply at every boundary | Platform | 18.PFF-FA-AI-GUARDRAILS.md; ADR-D6-09 |
...
**Weaknesses.**
- Fails DR-N-02 and 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8 outright. An agent can reach the SLM directly, and then guardrails,
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50's eleven stages do not all apply at the same frequency. Applying them all per node
would be wasteful; applying them all per run would leave most operations unguarded. They are
therefore split by scope:

| Stage (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50) | Scope | Rationale |
|---|---|---|
| Security validation | **Per run**, on entry | Claims and archetype do not change mid-run |
| Context validation | **Per context read** | Each ERC read is validated on retrieval (8 PFF-FA-AI-ERC-CONTEXT.md §67) |
...
| Limits and loop protection | **Continuous across the run** | Cumulative by nature (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §73) |
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72–§73 and 4. PFF-FA-AI-RUNTIME.md §54 require loop protection and runtime limits. These are cumulative
...
| Same tool with same parameters, repeated | Run | Terminate — a loop, not progress (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §73) |
...
1. **Limits** — has this run exceeded its tool-call budget? Has this exact call been made before
   with these parameters (7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §73)?
2. **Tool policy** — is `submit_affiliation` in this agent's allowlist? (I-3)
3. **Parameter validation** — do the parameters satisfy the tool's schema? (I-3, 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §35)
4. **Authorization** — do the harness-held claims permit this operation on this resource? Model
   output is not consulted. (I-2)
5. **Idempotency** — is an idempotency key required and present? (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47)
6. **Execution** — dispatch through the tool executor with timeout and retry per ADR-D2-11.
7. **Result validation** — does the response satisfy the tool's response schema? (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §36)
...
- 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §50's eleven stages are adopted with §7.2's scoping, which the specification leaves open.
...
|---|---|
| Enterprise decides; AI orchestrates | The harness is where 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8's requirement is realised: the SLM controls no runtime operation directly. Every enterprise reach passes tool policy, parameter validation and claims-based authorization. |
...
| Repository paths | `src/pff_fa_ai/orchestration/harness/` |
...
| Specification sections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §8 (Agent Harness Responsibility), §50 (Agent Harness Execution Pipeline), §61 (Guardrail Architecture), §67–§69 (Reasoning Boundary, Deterministic Control Boundary, AI Reasoning Boundary), §72–§73 (Agent Loop, Loop Protection), §5 (agent prohibitions); 1 PFF-FA-AI-ARCHITECTURE.md §11 (Agent Harness); 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §10 (Agent Harness), §3.3 (Deterministic Control); 4. PFF-FA-AI-RUNTIME.md §16 (Agent Harness Initialization), §54 (Runtime Limits), §55–§56 (Timeout and Retry Hierarchy) |
| Requirement IDs | `FR-A39-03`, `FR-A39-07`, `FR-A39-11`, `NFR-A38-SEC` |
| Build phases | 4 |
| Code paths | `src/pff_fa_ai/orchestration/harness/` |
