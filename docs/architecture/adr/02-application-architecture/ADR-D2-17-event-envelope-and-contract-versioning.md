...
Doc 11 §11–§24 specify the event contract in detail: contract principles, envelope, event ID,
type, version, source, timestamp, correlation ID, causation ID, workflow instance ID, entity
identity, tenant and organisation context, and payload. Doc 11 §36–§37 cover event and schema
validation. Doc 11 §38 covers unknown event version and §39 unknown event type. Doc 20 §73–§74
...
**What does "unknown version" mean in practice?** Doc 11 §38 requires handling but not what the
...
**How much does the platform depend on envelope fields it does not control?** Doc 11 §21's
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Events must carry a defined envelope | doc 11 §13–§24 |
| DR-F-02 | Events must be schema-validated | doc 11 §36–§37 |
| DR-F-03 | Unknown versions and types must be handled explicitly | doc 11 §38–§39 |
| DR-F-04 | Correlation and causation must propagate | doc 11 §19–§20; ADR-D7-03 |
| DR-F-05 | Workflow instance identity must resolve a suspended workflow | doc 11 §21; ADR-D2-10 |
| DR-F-06 | Tenant and organisation context must be present | doc 11 §23 |
...
| DR-N-02 | A contract change must be detectable | 0 undetected breaking changes | doc 20 §74 |
| DR-N-03 | Schemas must be versioned as artefacts | Immutable per version | doc 20 §73 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | The enterprise owns event contracts | Organisational | doc 3 §28 |
| DR-C-02 | An event is a notification, not data | Platform | ADR-D2-03 §7.4 |
| DR-C-03 | Payload content must never become prompt instruction | Platform | doc 11 §58 |
| DR-C-04 | Versioned artefacts are immutable | Platform | doc 20 §73; ADR-D0-02 §7.3 |
...
| EC-04 | Schema artefact governance | 15 | Doc 20 §73 requires versioned immutable artefacts | Are schemas versioned and immutable? |
...
- An unknown version is immediately visible as a dead-letter with a specific reason, detected in
  minutes (EC-02).
- Schemas are versioned, immutable, diffable and reviewable, satisfying doc 20 §73 (EC-04).
...
A schema file is **immutable once merged**, per doc 20 §73 and ADR-D0-02's identity principle. A
...
Per doc 11 §13–§24, every event carries:

| Field | Doc 11 | Platform use |
|---|---|---|
...
| `timestamp` | §18 | Staleness detection (doc 11 §47) |
...
interpreted — which is part of what closes doc 11 §58's injection path (ADR-D2-03 §7.4).
...
monitored per ADR-D2-18, and can be replayed after the schema is added. Doc 11 §39's requirement
...
`workflow_instance_id` (doc 11 §21) is the resumption key. It exists only if the enterprise
...
content usable as prompt content. Per ADR-D2-03 §7.4 and doc 11 §58, handlers extract typed
...
| Versioned artefacts, never mutated in place | §7.1: schema files are immutable once merged; a change is a new version. This is doc 20 §73 applied to event contracts. |
...
| Data classification touched | Envelopes carry identifiers and tenant context; payloads carry identifiers by design (doc 11 §24). |
...
| UK GDPR lawful basis and rights impact | Tenant and organisation context (doc 11 §23) enforces isolation; typed payloads support minimisation. |
| Audit and evidential requirements | `correlation_id` and `causation_id` give an unbroken chain from enterprise decision to platform action (doc 20 §71's HIL evidence). |
...
| Specification sections | doc 11 §11–§12 (Event Contract, Principles), §13 (Envelope), §14–§18 (ID, Type, Version, Source, Timestamp), §19–§20 (Correlation, Causation), §21 (Workflow Instance ID), §22–§23 (Entity Identity, Tenant Context), §24 (Payload), §36–§37 (Event and Schema Validation), §38 (Unknown Event Version), §39 (Unknown Event Type), §58; doc 20 §73–§74 (Version Governance, Compatibility); `DEVELOPMENT-GUIDE.md` §3 |
