...
source_docs:
  - "MD files/3 Context & Integration/10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §41, §42, §43, §44, §45, §46, §47, §48, §49"
  - "MD files/1 Foundation/4. PFF-FA-AI-RUNTIME.md §37, §54, §55, §56"
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72, §73"
  - "MD files/1 Foundation/3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §36, §37"
build_phases: [4, 6]
impacted_paths:
...
and a write is retried only when it carries an idempotency key. 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48's "blind transaction
...
10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §41–§44 cover retry policy, retryable and non-retryable examples, and retry configuration.
10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47 cover idempotency, keys and lifecycle. 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §48–§49 cover unknown transaction
state and transaction verification. 4. PFF-FA-AI-RUNTIME.md §55 gives a timeout hierarchy and §56 a retry hierarchy.
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72–§73 give the agent loop and loop protection. 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §36–§37 assign transaction and
idempotency responsibility. 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 lists blind transaction retry among the anti-patterns.
...
**What makes a write safely retryable?** 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47 give idempotency keys. But an idempotency
...
**How do the timeout and retry hierarchies compose?** 4. PFF-FA-AI-RUNTIME.md §55 and §56 give both without stating
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Blind transaction retry must never occur | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 |
| DR-F-02 | Retryable and non-retryable operations must be distinguished | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §41–§43 |
| DR-F-03 | Idempotency keys must be used for write operations | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§46 |
| DR-F-04 | Unknown transaction state must be handled explicitly | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §48–§49 |
| DR-F-05 | Loop protection must terminate runaway agent runs | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §73; 4. PFF-FA-AI-RUNTIME.md §54 |
| DR-F-06 | Timeout and retry hierarchies must be defined | 4. PFF-FA-AI-RUNTIME.md §55–§56 |
...
| DR-N-03 | Duplicate enterprise operations must not occur | 0 duplicates | 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §37 |

### 3.3 Constraints

| ID | Constraint | Type | Source |
|---|---|---|---|
| DR-C-01 | Transaction authority is enterprise-owned | Platform | ADR-D1-01 §7.2; 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §36 |
| DR-C-02 | The platform must not guess at ambiguous transaction outcomes | Platform | `CLAUDE.md`; 8 PFF-FA-AI-ERC-CONTEXT.md §66 |
| DR-C-03 | Retry configuration is declared, not inferred | Platform | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §10, §44 |
| DR-C-04 | Critical controls are deterministic | Platform | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.3 |
...
|---|---|---|---|---|
| EC-01 | Prevention of duplicate enterprise operations | 35 | A duplicate affiliation application or a double payment is the worst outcome the platform can cause; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 makes it categorical | Can a retry create a duplicate? |
...
**Weaknesses.**
- Retries writes blindly, which is 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48's named anti-pattern. A retried `submit_affiliation`
...
- Unknown is a first-class outcome resolved by verification, matching 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §48–§49 (EC-02).
...
- 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.3 requires deterministic critical controls; transaction safety is as critical as it
  gets.
...
the criterion carrying 35 points and reflecting a 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 categorical prohibition — no
...
Each catalogue entry declares, per 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §10 and §44:
...
intuitive response — retry, since it was only a timeout — is exactly the blind retry 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48
forbids, because a timeout means the request may have been processed.

### 7.3 Idempotency keys

Per 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47, a write declared `idempotency_key_required` carries a key derived
...
10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §48–§49 and 8 PFF-FA-AI-ERC-CONTEXT.md §66 identify the unknown state. The handling:
...
4. PFF-FA-AI-RUNTIME.md §55 and §56 give timeout and retry hierarchies. The invariant between them is stated here
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §73 requires loop protection. The harness enforces cumulative limits per run (ADR-D2-09
...
|---|---|
| Enterprise decides; AI orchestrates | §7.4's verification asks the enterprise what happened rather than the platform deciding what probably happened. Transaction authority stays with the enterprise per 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §36. |
...
QM-01, QM-02, QM-04 and QM-06 carry zero thresholds — each is a categorical breach of a 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md
...
| Repository paths | `src/pff_fa_ai/integration/execution/retry.py`, `timeout.py`, `idempotency.py`, `circuit.py`; `src/pff_fa_ai/orchestration/harness/` |
...
| Specification sections | 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §41–§44 (Retry Policy, Retryable/Non-Retryable Examples, Retry Configuration), §45–§47 (Idempotency, Key, Lifecycle), §48 (Unknown Transaction State), §49 (Transaction Verification), §10 (Extended Metadata); 4. PFF-FA-AI-RUNTIME.md §37 (Transaction Safety), §54 (Runtime Limits), §55 (Timeout Hierarchy), §56 (Retry Hierarchy); 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §72–§73 (Agent Loop, Loop Protection); 3. PFF-FA-AI-RESPONSIBILITY-MATRIX.md §36 (Transaction Responsibility), §37 (Idempotency Responsibility); 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 (Anti-Patterns — blind transaction retry); 8 PFF-FA-AI-ERC-CONTEXT.md §66 (Transaction Uncertainty); affiliation flow Scenarios 21–27 |
| Requirement IDs | `NFR-A38-REL`, `NFR-A38-RECOV` |
| Build phases | 4, 6 |
| Code paths | `src/pff_fa_ai/integration/execution/` |
