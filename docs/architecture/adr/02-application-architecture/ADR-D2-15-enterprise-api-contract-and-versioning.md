...
rather than something to tolerate, because doc 10 §22 requires the mapping to be deterministic
and a tolerated surprise is a silent behaviour change.

## 2. Context and Problem Statement

Doc 10 §15–§24 cover API versioning, contracts, request and response payloads,
response-to-context mapping, why the mapping must be deterministic, data transformation and raw
response handling. Doc 10 §36 covers tool output validation and §38–§40 the error contract and
its categories. Doc 3 §67–§68 assign responsibility for payload transformation and version
compatibility. Doc 20 §74 covers version compatibility governance.

Doc 10 §22 is unusually direct: the response-to-context mapping *must* be deterministic. It does
not say what happens when a response does not fit the mapping, and that gap is where the real
decision sits.

Three related problems.

**Raw responses are dangerous in a way that is easy to miss.** Doc 10 §24 covers raw response
...
platform is stable and requires deliberate work to adopt changes. Doc 10 §15 requires versioning
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Response-to-context mapping must be deterministic | doc 10 §21–§22 |
| DR-F-02 | API versions must be explicit | doc 10 §15 |
| DR-F-03 | Tool and API outputs must be validated | doc 10 §36 |
| DR-F-04 | Errors must map to a platform error contract | doc 10 §38–§40 |
| DR-F-05 | Payload transformation responsibility is the platform's | doc 3 §67 |
| DR-F-06 | Version compatibility must be governed | doc 3 §68; doc 20 §74 |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | A contract change must be detected before it reaches users | 0 undetected breaking changes | doc 20 §74 |
...
|---|---|---|---|
| DR-C-01 | Enterprise owns its API contracts | Organisational | doc 3 §4; ADR-D2-14 §7.3 |
...
| EC-02 | Determinism of mapping | 25 | Doc 10 §22 requires it; non-deterministic mapping makes ERC untrustworthy | Same response, same ERC content, always? |
...
- Mapping is not deterministic in the doc 10 §22 sense — what ends up in ERC depends on what the
...
Each catalogue entry names an explicit API version (doc 10 §9's `version: v1`). The platform never
...
and a release. Doc 20 §74's version-compatibility governance applies. The old version continues to
...
Doc 3 §67 assigns payload transformation to the platform, and this is what that means in practice:
...
Per doc 10 §38–§40, enterprise errors are translated into the platform's own error taxonomy
...
- Mapping is explicit and deterministic, satisfying doc 10 §22.
...
| Versioned artefacts, never mutated in place | API versions pinned; contracts versioned with the catalogue (ADR-D5-06); adoption is a release, per doc 20 §74. |
...
| Specification sections | doc 10 §15 (API Versioning), §16–§20 (API Contract, Request/Response Contract and Payload), §21 (Response-to-Context Mapping), §22 (Why Mapping Must Be Deterministic), §23–§24 (Data Transformation, Raw Response Handling), §36 (Tool Output Validation), §38–§40 (Error Contract, Translation, Categories); doc 3 §67 (Responsibility for API Payload Transformation), §68 (Responsibility for Version Compatibility); doc 20 §74 (Version Compatibility) |
