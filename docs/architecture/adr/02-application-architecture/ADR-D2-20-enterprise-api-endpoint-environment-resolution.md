...
source_docs:
  - "MD files/4 AI/17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §31, §32, §33, §34, §35"
  - "MD files/3 Context & Integration/10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §7, §8, §9, §10, §15, §16"
  - "MD files/6 Production/25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §33, §34, §35, §36, §37, §38, §39"
build_phases: [1, 6]
impacted_paths:
...
configuration (17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33), owned by ADR-D5-06's config model and never hard-coded in
...
17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §31–§32 define enterprise API catalogue configuration and API versioning
...
architecture). 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md's own catalogue metadata example (§10) shows `endpoint: {method,
path}` with a relative path and no host, which is consistent with 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33's
indirection but never says so — a reader of 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md alone would not know where the host
...
2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 lists hard-coded URLs among the platform's anti-patterns; ADR-D2-19 already
enforces that discipline for portal links, with its own environment-resolution decision
(12 PFF-FA-AI-PORTAL-LINKS.md §7–§10). Enterprise API endpoints carry the identical risk — a DEV base URL
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Endpoint location must be declared separately from the business contract | 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 |
| DR-F-02 | The catalogue entry (`api_id`, version, operation) must be environment-agnostic | 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §31–§32; ADR-D5-06 |
| DR-F-03 | The physical `base_url` per `endpoint_ref` must resolve per environment | 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 |
| DR-F-04 | Endpoint (URL) version and API contract version must be tracked independently | 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §34 |
| DR-F-05 | No API host/URL is hard-coded in agent, tool or catalogue code | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48; ADR-D2-19 (parallel) |

### 3.2 Non-functional drivers

| ID | Driver | Target | Source |
|---|---|---|---|
| DR-N-01 | No cross-environment enterprise call | 0 occurrences | 25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md §39 |
...
| EC-03 | No hard-coded host in code | 20 | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 anti-pattern | Static analysis for literal hosts |
...
### 5.2 Option B — `endpoint_ref` indirection; environment config resolves `base_url` (17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33)
...
exactly what 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 already specifies — adopting it is recognising an existing
...
17.PFF-FA-AI-CONFIGURATION-VERSIONING.md's static model does not have; a misconfigured DNS zone silently crosses
environments, which is precisely EC-01's worst case, with no config diff to catch it;
disproportionate for an integration surface 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §7 already catalogues explicitly and
...
to accept; no evidence in 10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md / 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md / 25.PFF-FA-AI-INFRASTRUCTURE-OPERATIONS.md that APIM is meant to carry this responsibility.
...
2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48's anti-pattern applies directly.
...
|---|---|
| Option E (code conditionals) | DR-F-05 — hard-coded environment logic is the anti-pattern 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §48 names |
...
specification already states (17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33), and D is a plausible but undocumented
...
Per 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §34, a `base_url` change (a service relocating) does not imply the request/
...
Where 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33's example shows `base_url: https://...` sourced from environment
...
review of the ADR library: 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 already specifies this mechanism, and ADR-D2-19
...
An enterprise service relocating (endpoint version bump, 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §34) triggers only an
...
- The mechanism is not new — it recognises 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 as already specifying it. This ADR
...
| Repository paths | `src/pff_fa_ai/integration/api/` (catalogue loader, `endpoint_ref` resolver); `config/` (per-environment endpoint maps) |
| Configuration | `endpoints.<ENDPOINT_REF>.base_url` per environment overlay (17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33) |
...
| AC-02 | No literal host string appears in `src/pff_fa_ai/integration/` | Static analysis (QM-03) |
...
| Per-environment endpoint config authoring | Small, per operation | Per new operation or relocation | 17.PFF-FA-AI-CONFIGURATION-VERSIONING.md §33 |
