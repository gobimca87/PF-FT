...
`CLAUDE.md` lists Azure Service Bus under Confirmed Tech Stack. Doc 11 covers it across sixty-odd
...
do, §50–§51 ERC refresh triggering and scope. Doc 3 §28 splits Service Bus responsibility:
...
**Does the platform ever produce events?** Doc 3 §28 assigns event production to the enterprise
...
signalling would put platform-internal messages on enterprise infrastructure, blur the doc 3 §28
split, and create a channel through which the platform could appear to be asserting business
facts.

**What does the platform subscribe to?** Doc 11 §30 covers subscription filtering without saying
...
**Who owns the subscription's lifecycle?** Doc 11 §31 covers subscription responsibility. If the
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | Events must refresh ERC and resume workflows | doc 1 §39 criterion 12; doc 11 §50–§51 |
| DR-F-02 | The enterprise produces; the AI consumes | doc 3 §28; doc 11 §2 |
| DR-F-03 | The consumer must not make business decisions | doc 11 §34 |
| DR-F-04 | Subscriptions must be filtered | doc 11 §30 |
| DR-F-05 | Four event types must be supported | doc 11 §6–§10 |
...
| DR-C-03 | An event is a notification, not data or instruction | Platform | ADR-D2-03 §7.4; doc 11 §25, §58 |
| DR-C-04 | The enterprise owns event contracts | Organisational | doc 3 §28 |
...
**Weaknesses.**
- Contradicts doc 3 §28's split (DR-F-02).
...
**Weaknesses.**
- Fails doc 1 §39 criterion 12, which requires event-driven refresh and resume.
...
of it is processing personal data without need. D is excluded by doc 1 §39 criterion 12 and by its
...
This is stronger than a policy. Doc 3 §28's split is realised by the platform being structurally
...
The platform owns its subscriptions per doc 11 §31. Filters express, as precisely as the broker
allows:
...
- **Schema versions** the platform supports, so an unsupported version is not delivered rather
  than being delivered and rejected (doc 11 §38).
...
Doc 11 §6–§10 define four, each with a distinct handling:

| Type | Doc 11 | Affiliation examples | Handling |
|---|---|---|---|
| **Workflow events** | §7 | Application status transitions | Invalidate ERC section; refresh; resume workflow |
| **HIL events** | §8 | CFA approval, rejection, cancellation, override | Same, plus HIL evidence recording (doc 20 §71) |
...
Doc 11 §34 constrains the consumer. Restated as what it may and may not do:
...
| Topics | Enterprise | Producers own their topics (doc 3 §28) |
| **AI subscription** | **PFF AI platform** | One per topic of interest; filters are platform configuration (doc 11 §29, §31) |
...
| Aspect | Decision |
|---|---|
| Authentication | Azure Managed Identity (doc 25 §28), never connection strings with embedded keys |
...
| Network | Private connectivity per doc 25 §24; no public endpoint traversal |
...
| Data classification touched | Event payloads carry identifiers by design (doc 11 §24), not personal data; refreshed API responses carry the personal data. |
...
| Audit and evidential requirements | Correlation and causation IDs (doc 11 §19–§20) link enterprise decision to platform action to user notification, giving an unbroken chain for HIL evidence (doc 20 §71). |
...
| Specification sections | doc 11 §2 (Core Principle), §3–§5 (Position, Why Required, Event-Driven Workflow Principle), §6–§10 (Event Types, Workflow, HIL, Enterprise Data, External), §19–§20 (Correlation, Causation), §24–§25 (Payload, Event as Notification), §27–§31 (Namespace, Topics, AI Subscription, Filtering, Subscription Responsibility), §32–§34 (Consumer Architecture, Responsibilities, Must Not), §38 (Unknown Event Version), §50–§51 (ERC Refresh Trigger, Scope), §58 (Event Should Not Become Prompt Instruction); doc 1 §24, §39 criterion 12; doc 3 §28 (Service Bus Responsibility); doc 25 §24, §28; doc 20 §71 |
