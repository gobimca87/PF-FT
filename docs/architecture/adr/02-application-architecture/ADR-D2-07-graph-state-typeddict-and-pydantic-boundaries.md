...
source_docs:
  - "MD files/2 Agent Runtime/7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26, §27, §40"
  - "MD files/1 Foundation/2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §12"
  - "MD files/1 Foundation/4. PFF-FA-AI-RUNTIME.md §20"
  - "MD files/1 Foundation/5. PFF-FA-AI-STATE-MODEL.md"
build_phases: [2, 4]
impacted_paths:
...
boundary is a Pydantic model. The consequential part is 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27's rule: graph state carries
...
a boundary... LangGraph internal state: TypedDict."* 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26 gives a conceptual
`AgentGraphState` with fourteen fields. 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 adds the rule that matters most:
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 justifies the rule on serialisation and token cost. Those are real, and they are the
...
**Personal data in workflow storage.** 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26's state includes `erc`, `rag_context`,
...
| DR-F-03 | Graph state must reference large data, not copy it | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27, §40 |
| DR-F-04 | Graph state must be strongly typed | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26 |
| DR-F-05 | State must be serialisable for suspension and resume | ADR-D2-10 |
| DR-F-06 | The four state concepts must not be conflated in graph state | `CLAUDE.md`; 5. PFF-FA-AI-STATE-MODEL.md |
...
| DR-N-02 | Persisted state must stay small | ≤ configured ceiling | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 |
...
- 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27's rule is directly violated.
...
- Satisfies `CLAUDE.md` and 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26–§27 directly.
...
| ERC sections | Pydantic | At construction and on validation (8 PFF-FA-AI-ERC-CONTEXT.md §67) |
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 justifies references on serialisation and token overhead. Three further consequences
...
7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26's conceptual state mixes references to all four state concepts. The typing keeps them
distinct:
...
reason §7.3 treats 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27 as binding.
...
| Tool result no longer stored | Status is retained on the reference itself, so completion logic works. The payload is refetched or the step re-run under idempotency (10 PFF-FA-AI-ENTERPRISE-INTEGRATION.md §45–§47). |
| Conversation closed | Workflow continues; the outcome reaches the user through a new conversation (6 PFF-FA-AI-CONVERSATION-SESSION.md §50). |
...
- The split is `CLAUDE.md`'s; this decision records why and adds the reference rule's fuller
  rationale.
- 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26's conceptual state is adopted with typing that enforces §7.4's separation.
...
| Repository paths | `src/pff_fa_ai/orchestration/langgraph/state.py`, `src/pff_fa_ai/domain/workflow/` |
...
| Specification sections | 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §26 (Graph State), §27 (Graph State Rule), §40 (ERC as Graph State Reference); 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §12 (LangGraph State); 4. PFF-FA-AI-RUNTIME.md §20 (LangGraph Runtime State); 5. PFF-FA-AI-STATE-MODEL.md (State Model); `CLAUDE.md` §Coding Conventions |
| Requirement IDs | `NFR-A38-MAINT`, `NFR-A38-SEC`, `NFR-A38-PERF` |
| Build phases | 2, 4 |
| Code paths | `src/pff_fa_ai/orchestration/langgraph/state.py`, `src/pff_fa_ai/domain/workflow/` |
| Configuration | Persisted-state size ceiling |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. `CLAUDE.md`'s representation split confirmed with the "validate where trust changes" principle; 7 PFF-FA-AI-AGENTIC-ORCHESTRATION.md §27's reference rule treated as binding on provenance, freshness and personal-data grounds rather than only serialisation cost; `claims` made read-only by type. |
