...
source_docs:
  - "MD files/3 Context & Integration/8 PFF-FA-AI-ERC-CONTEXT.md §2, §3, §4, §5, §6, §7, §8, §9, §20, §21, §22, §30"
  - "MD files/1 Foundation/1 PFF-FA-AI-ARCHITECTURE.md §14, §15"
  - "MD files/1 Foundation/2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.5, §14, §15"
build_phases: [5]
impacted_paths:
...
the authoritative-truth precedence chain and 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.5's "context before reasoning" from
principles into properties.

## 2. Context and Problem Statement

8 PFF-FA-AI-ERC-CONTEXT.md is the platform's longest specification document, and its first seven sections are
...
1 PFF-FA-AI-ARCHITECTURE.md §14–§15 and 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §14–§15 give ERC architecture and processing pipeline. 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.5 states
...
a prompt, or merely the **primary** one. 8 PFF-FA-AI-ERC-CONTEXT.md §2 says ERC "is the primary operational context
...
- **Context budget** (8 PFF-FA-AI-ERC-CONTEXT.md's projection stage) bounds what reaches the model. A bypass path is
  unbudgeted.
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | ERC aggregates multiple enterprise APIs into structured context | 1 PFF-FA-AI-ARCHITECTURE.md §39 criterion 5; 8 PFF-FA-AI-ERC-CONTEXT.md §30 |
| DR-F-02 | ERC is not the enterprise database, memory, cache or RAG | 8 PFF-FA-AI-ERC-CONTEXT.md §4–§7 |
| DR-F-03 | Context requirements must be identified before collection | 8 PFF-FA-AI-ERC-CONTEXT.md §22–§23 |
| DR-F-04 | Every fact must carry provenance and authority | 8 PFF-FA-AI-ERC-CONTEXT.md §15–§16, §19; ADR-D1-03 |
| DR-F-05 | Context must be bounded, not an uncontrolled dataset | 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.5; 8 PFF-FA-AI-ERC-CONTEXT.md §8 |
| DR-F-06 | ERC has identity, version and lifecycle | 8 PFF-FA-AI-ERC-CONTEXT.md §9–§13 |
...
| DR-N-02 | Context must fit the model's budget | Within the projection budget | 8 PFF-FA-AI-ERC-CONTEXT.md projection |
...
| DR-C-02 | RAG is knowledge, never operational truth | Platform | ADR-D1-03 §7.2; 8 PFF-FA-AI-ERC-CONTEXT.md §7 |
| DR-C-03 | Precedence is computed from fact-level authority | Platform | ADR-D1-03 §8.1 |
| DR-C-04 | Graph state references ERC rather than copying it | Platform | ADR-D2-07 §7.2; 8 PFF-FA-AI-ERC-CONTEXT.md §40 |
...
- Matches a literal reading of 8 PFF-FA-AI-ERC-CONTEXT.md §2's "primary".
...
- Implements 8 PFF-FA-AI-ERC-CONTEXT.md §4–§7's negative definitions as a structural property rather than a caution.
...
- 8 PFF-FA-AI-ERC-CONTEXT.md §60–§61's ERC update and patch model already covers mid-reasoning updates, so the second
...
**Weaknesses.**
- This is precisely what 8 PFF-FA-AI-ERC-CONTEXT.md §6 says ERC is not. A cache is keyed by request; ERC is a
  structured, versioned, provenance-bearing projection with a lifecycle (8 PFF-FA-AI-ERC-CONTEXT.md §9–§13).
...
- Cache authority is 3, below ERC's 4 (ADR-D1-03 §7.1); collapsing them would flatten the
  precedence chain.
- Loses ERC's identity, versioning and completeness tracking, which 8 PFF-FA-AI-ERC-CONTEXT.md §51–§52 require.
...
cannot carry a design whose purpose is guaranteeing properties. D is excluded by 8 PFF-FA-AI-ERC-CONTEXT.md §6
...
8 PFF-FA-AI-ERC-CONTEXT.md §2's "primary" is read as "exclusive" for operational truth. The reasoning: the properties
...
2. The result **updates ERC** via the patch model (8 PFF-FA-AI-ERC-CONTEXT.md §61), stamped with provenance:
...
and the provenance records that — which is better than being fresher and unranked. 8 PFF-FA-AI-ERC-CONTEXT.md §36
("ERC Update After Tool Execution", 4. PFF-FA-AI-RUNTIME.md §36) already describes this flow; this decision makes
it mandatory rather than typical.

### 7.3 What ERC is not — with the consequence of each confusion

8 PFF-FA-AI-ERC-CONTEXT.md §4–§7's four negative definitions, each with what breaks if ignored:
...
| **RAG** (§7) | It carries operational truth; RAG carries knowledge | RAG content could answer an operational question, which 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.6 forbids |
...
Per 8 PFF-FA-AI-ERC-CONTEXT.md §22–§23, each workflow step declares what context it needs — which sections, which
...
8 PFF-FA-AI-ERC-CONTEXT.md §9–§13 give ERC identity, version and lifecycle. ERC is scoped to a workflow instance within
a conversation and persists across turns, updated by patch (8 PFF-FA-AI-ERC-CONTEXT.md §61) and invalidated by
freshness policy or event (8 PFF-FA-AI-ERC-CONTEXT.md §62–§64).
...
**Status rationale.** Accepted. Tier 1 under ADR-D0-03 §7.1 — ERC is a named 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §52 category —
...
flowchart TD
    ...
    subgraph ERC["ERC — the only entry for operational truth"]
        ...
        PLAN --> COL[Collection]
        COL --> NORM[Normalisation<br/>8 PFF-FA-AI-ERC-CONTEXT.md §32-§33]
        NORM --> PROV[Provenance stamping<br/>authority, freshness, source]
        PROV --> SEC[ERC sections<br/>versioned, completeness-tracked]
        SEC --> PROJ[Projection + budget<br/>8 PFF-FA-AI-ERC-CONTEXT.md projection]
    end
...
- 8 PFF-FA-AI-ERC-CONTEXT.md §2's "primary" is read as exclusive, which is an interpretation the specification permits
  but does not compel.
...
| QM-06 | ERC section completeness on assembly | ≥98% for mandatory sections | <95% | Completeness tracking (8 PFF-FA-AI-ERC-CONTEXT.md §51) | Weekly |
...
| Audit and evidential requirements | ERC identity, version and provenance give a complete account of what the platform knew, from which source, at what time — 20.PFF-FA-AI-GOVERNANCE.md §60's data lineage requirement satisfied structurally. |
...
|---|---|
| Build phases | 5 (ERC) |
| Repository paths | `src/pff_fa_ai/context/erc/`, `collection/`, `normalization/`, `projection/` |
...
| RT-05 | 8 PFF-FA-AI-ERC-CONTEXT.md §2's "primary" is clarified by the enterprise as permitting exceptions | Change notice | Re-evaluate §7.1; the exclusivity reading is the strong one and would need explicit relaxation |
...
| Specification sections | 8 PFF-FA-AI-ERC-CONTEXT.md §2 (Core Principle), §3 (What ERC Means), §4 (Not the Enterprise Database), §5 (Not Conversation Memory), §6 (Not Cache), §7 (Not RAG), §8 (ERC Scope), §9–§13 (Lifecycle, Status, Identity, Version, Schema Version), §15–§16 (Provenance), §19 (Authority Levels), §20–§21 (Structure, Dynamic Sections), §22–§23 (Context Requirements), §30 (Construction Pipeline), §32–§33 (Normalisation, Transformation), §51–§52 (Completeness), §59–§64 (Versioning, Update, Patch, Invalidation, Refresh); 1 PFF-FA-AI-ARCHITECTURE.md §14–§15, §39 criterion 5; 2. PFF-FA-AI-ARCHITECTURE-DETAILED.md §3.5 (Context Before Reasoning), §3.6, §14–§15; 4. PFF-FA-AI-RUNTIME.md §36 (ERC Update After Tool Execution) |
| Requirement IDs | `FR-A39-05`, `NFR-A38-SEC`, `NFR-A38-PERF` |
| Build phases | 5 |
| Code paths | `src/pff_fa_ai/context/` |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. 8 PFF-FA-AI-ERC-CONTEXT.md §2's "primary" read as exclusive for operational truth, because provenance, freshness and budget are properties of the entry path and cannot be added retrospectively; tool results routed through ERC; 8 PFF-FA-AI-ERC-CONTEXT.md §4–§7's four negative definitions given their concrete consequences. Tier 1 — ratified by the external ADF/ADR forum. |
