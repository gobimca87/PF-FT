...
the authoritative-truth precedence chain and doc 2 §3.5's "context before reasoning" from
principles into properties.

## 2. Context and Problem Statement

Doc 8 is the platform's longest specification document, and its first seven sections are
...
Doc 1 §14–§15 and doc 2 §14–§15 give ERC architecture and processing pipeline. Doc 2 §3.5 states
...
a prompt, or merely the **primary** one. Doc 8 §2 says ERC "is the primary operational context
...
- **Context budget** (doc 8's projection stage) bounds what reaches the model. A bypass path is
  unbudgeted.
...
| ID | Driver | Source |
|---|---|---|
| DR-F-01 | ERC aggregates multiple enterprise APIs into structured context | doc 1 §39 criterion 5; doc 8 §30 |
| DR-F-02 | ERC is not the enterprise database, memory, cache or RAG | doc 8 §4–§7 |
| DR-F-03 | Context requirements must be identified before collection | doc 8 §22–§23 |
| DR-F-04 | Every fact must carry provenance and authority | doc 8 §15–§16, §19; ADR-D1-03 |
| DR-F-05 | Context must be bounded, not an uncontrolled dataset | doc 2 §3.5; doc 8 §8 |
| DR-F-06 | ERC has identity, version and lifecycle | doc 8 §9–§13 |
...
| DR-N-02 | Context must fit the model's budget | Within the projection budget | doc 8 projection |
...
| DR-C-02 | RAG is knowledge, never operational truth | Platform | ADR-D1-03 §7.2; doc 8 §7 |
| DR-C-03 | Precedence is computed from fact-level authority | Platform | ADR-D1-03 §8.1 |
| DR-C-04 | Graph state references ERC rather than copying it | Platform | ADR-D2-07 §7.2; doc 8 §40 |
...
- Matches a literal reading of doc 8 §2's "primary".
...
- Implements doc 8 §4–§7's negative definitions as a structural property rather than a caution.
...
- Doc 8 §60–§61's ERC update and patch model already covers mid-reasoning updates, so the second
...
**Weaknesses.**
- This is precisely what doc 8 §6 says ERC is not. A cache is keyed by request; ERC is a
  structured, versioned, provenance-bearing projection with a lifecycle (doc 8 §9–§13).
...
- Cache authority is 3, below ERC's 4 (ADR-D1-03 §7.1); collapsing them would flatten the
  precedence chain.
- Loses ERC's identity, versioning and completeness tracking, which doc 8 §51–§52 require.
...
cannot carry a design whose purpose is guaranteeing properties. D is excluded by doc 8 §6
...
Doc 8 §2's "primary" is read as "exclusive" for operational truth. The reasoning: the properties
...
2. The result **updates ERC** via the patch model (doc 8 §61), stamped with provenance:
...
and the provenance records that — which is better than being fresher and unranked. Doc 8 §36
("ERC Update After Tool Execution", doc 4 §36) already describes this flow; this decision makes
it mandatory rather than typical.

### 7.3 What ERC is not — with the consequence of each confusion

Doc 8 §4–§7's four negative definitions, each with what breaks if ignored:
...
| **RAG** (§7) | It carries operational truth; RAG carries knowledge | RAG content could answer an operational question, which doc 2 §3.6 forbids |
...
Per doc 8 §22–§23, each workflow step declares what context it needs — which sections, which
...
Doc 8 §9–§13 give ERC identity, version and lifecycle. ERC is scoped to a workflow instance within
a conversation and persists across turns, updated by patch (doc 8 §61) and invalidated by
freshness policy or event (doc 8 §62–§64).
...
**Status rationale.** Accepted. Tier 1 under ADR-D0-03 §7.1 — ERC is a named doc 2 §52 category —
...
- Doc 8 §2's "primary" is read as exclusive, which is an interpretation the specification permits
  but does not compel.
...
| Four-state separation | ERC is the platform's projection of Enterprise Business State, explicitly distinct from conversation, session and workflow state — which is what doc 8 §5's negative definition protects. |
| Versioned artefacts, never mutated in place | ERC sections are versioned (doc 8 §59) and updated by patch (doc 8 §61), not overwritten in place. |
...
| QM-06 | ERC section completeness on assembly | ≥98% for mandatory sections | <95% | Completeness tracking (doc 8 §51) | Weekly |
...
| Audit and evidential requirements | ERC identity, version and provenance give a complete account of what the platform knew, from which source, at what time — doc 20 §60's data lineage requirement satisfied structurally. |
...
| RT-05 | Doc 8 §2's "primary" is clarified by the enterprise as permitting exceptions | Change notice | Re-evaluate §7.1; the exclusivity reading is the strong one and would need explicit relaxation |
...
| Specification sections | doc 8 §2 (Core Principle), §3 (What ERC Means), §4 (Not the Enterprise Database), §5 (Not Conversation Memory), §6 (Not Cache), §7 (Not RAG), §8 (ERC Scope), §9–§13 (Lifecycle, Status, Identity, Version, Schema Version), §15–§16 (Provenance), §19 (Authority Levels), §20–§21 (Structure, Dynamic Sections), §22–§23 (Context Requirements), §30 (Construction Pipeline), §32–§33 (Normalisation, Transformation), §51–§52 (Completeness), §59–§64 (Versioning, Update, Patch, Invalidation, Refresh); doc 1 §14–§15, §39 criterion 5; doc 2 §3.5 (Context Before Reasoning), §3.6, §14–§15; doc 4 §36 (ERC Update After Tool Execution) |
...
|---|---|---|---|
| 1.0.0 | 2026-08-21 | AI Solution Architect | Initial decision recorded. Doc 8 §2's "primary" read as exclusive for operational truth, because provenance, freshness and budget are properties of the entry path and cannot be added retrospectively; tool results routed through ERC; doc 8 §4–§7's four negative definitions given their concrete consequences. Tier 1 — ratified by the external ADF/ADR forum. |
