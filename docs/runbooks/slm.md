...
2. Check circuit state — `SlmService` (`src/pf_ft_ai/slm/service.py`) owns one
   `CircuitBreaker` per instance; a P2 "SLM degraded" alert usually means it's
   `OPEN` (`CircuitState.OPEN`, `src/pf_ft_ai/integration/execution/circuit.py`).
...
4. If self-hosted (not yet in production — doc 15/ADR): check GPU/VRAM/model-loading
   ...
   Hugging Face providers are implemented — `src/pf_ft_ai/slm/providers.py`).
