...
|---|---|---|---|
| Model serving plane (SLM + embeddings) | **Azure AI Foundry** hosted-first (in-tenancy Azure, Entra ID/MI, Private Link) → self-hosted vLLM on AKS GPU as target; evaluation also in-tenancy on Foundry (no active external SLM path; HF a dormant adapter); all behind the `ADR-D3-14` abstraction | `ADR-D3-29` (supersedes `ADR-D3-13`) | ARB sign-off (DPO on placement/masking, FinOps on cost) + Phase 6 model eval |
