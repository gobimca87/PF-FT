...
from pf_ft_ai.cache.models import CacheEntry
...
def _build_key(
    *, environment: str, category: str, tenant_id: str, organization_id: str | None, key: str
    ...
    return f"pf-ft:{environment}:cache:{category}:{tenant_id}:{organization_segment}:{key}"
...
class RedisCacheStore:
    ...
    async def invalidate(
        ...
        pattern = f"pf-ft:{self._environment}:cache:{category.value}:{tenant_id}:*{key_prefix}*"
        deleted = 0
